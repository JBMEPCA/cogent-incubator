...
export async function launchProgress(siteId, { hasWordPress = false } = {}) {
  const db = forSite(siteId);
  const [published, inPipeline, topics, linkedInQueued, seoApplied, leads, advertisers, agentRuns] =
    await Promise.all([
      db.article.count({ where: { status: "published" } }),
      db.article.count({ where: { status: { in: ["drafting", "review", "approved"] } } }),
      db.researchTopic.count(),
      db.linkedInPost.count(),
      db.seoSuggestion.count({ where: { status: "applied" } }),
      db.lead.count(),
      db.advertiserProspect.count(),
      db.agentRun.count(),
    ]);

  // Was Boolean(process.env.WP_URL). The WordPress connection is now a property
  // of the title, so the caller passes it in from the site context.
  const siteLive = hasWordPress;
  ...
  const items = [
    { phase: "brand", label: "Domain live", done: siteLive, evidence: siteLive ? "WordPress reachable" : "no WordPress credential" },
