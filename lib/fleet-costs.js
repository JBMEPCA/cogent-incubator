...
export async function fleetCosts({ days = 30 } = {}) {
  ...
  const [byAgent, monthBySite, publishedBySite, runsBySite] = await Promise.all([
    db.agentRun.groupBy({
      by: ["siteId", "agentKey"],
  ...
  for (const site of sites) {
    ...
    titles.push({
      ...
      costPerArticleUsd: published ? (modelMonthUsd + fixedUsd + perTitleShareUsd) / published : null,
    });
  }
  ...
  return {
    ...
    totals: {
      ...
      publishedThisMonth,
      costPerArticleUsd: publishedThisMonth
        ? (modelMonthUsd + titleFixedUsd + fleetMonthlyUsd) / publishedThisMonth
        : null,
    },
  };
}
