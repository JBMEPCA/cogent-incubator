...
export async function addLinkedInPost(site, formData) {
  ...
  // Same rule as the agent's drafts: no slot, no post.
  await db.linkedInPost.create({ data: { text, scheduledFor: await nextPostingSlot(site) } });
...
export async function advanceLinkedInPost(site, formData) {
  ...
  if (next === "approved") {
    ...
    if (connection && !connection.expired) {
      // Keep the slot it was drafted into. Re-booking on approval would shunt
      // an already-scheduled post to the back of the queue for no reason, and
      // approving is no longer what triggers a send.
      data.scheduledFor = post.scheduledFor || (await nextPostingSlot(site));
...
export async function disconnectLinkedIn(site) {
  ...
  // Called with no argument until 1 Sep 2026, so disconnect() threw on
  // site.id and the button did nothing but show an error page.
  await disconnect(site);
  revalidatePath(sitePath(site, "/linkedin"));
}

// Bind one of the connected account's company pages to this title. Separate
// from connecting because they are separate decisions: one LinkedIn sign-in
// can administer every title's page, and which page THIS title posts as is
// not something the OAuth handshake can answer.
export async function selectLinkedInOrganisation(site, formData) {
  await requireEditor();
  const { selectOrganisation } = await import("./linkedin");
  await selectOrganisation(site, formData.get("urn"));
  revalidatePath(sitePath(site, "/linkedin"));
}
...
// A link we know about that nothing can see yet.
//
// GA4 only finds a link once a reader clicks it, and the brand crawler only
// looks at brands we emailed, so a link spotted by eye had nowhere to go and
// was simply not counted. firstSeenAt is left at today rather than asked for:
// a guessed date on a chart of when links arrived is worse than an honest one.
export async function recordBacklink(site, formData) {
  await requireEditor();
  const db = forSite(site.id);
  ...
  await db.referringDomain.upsert({
    where: { domain },
    create: { domain, linkUrl: url.toString(), source: "manual" },
    // An existing row means GA4 already found the domain. Adding the URL by
    // hand fills in the one thing GA4 could never tell us, so it updates the
    // link without resetting the date GA4 first saw it or un-ignoring it.
    update: { linkUrl: url.toString() },
  });
  revalidatePath(sitePath(site, "/outreach"));
}

// Not every referring domain is a link worth counting — a payment hop, a
// staging URL, an aggregator scraping the feed. Hidden rather than deleted:
// the sweep would find a deleted row again tomorrow and report it as new.
export async function ignoreReferrer(site, formData) {
  await requireEditor();
  const db = forSite(site.id);
  const domain = formData.get("domain");
  if (!domain) return;
  await db.referringDomain.update({
    where: { domain },
    data: { ignored: formData.get("undo") ? false : true },
  });
  revalidatePath(sitePath(site, "/outreach"));
}
