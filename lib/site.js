...
import { decryptJson, encryptJson, redact, isCryptoConfigured } from "./crypto";
...
export const CREDENTIAL_KINDS = {
  ...
  linkedin: {
    label: "LinkedIn",
    fields: ["organisationUrn", "accessToken", "refreshToken", "expiresAt"],
    required: false,
    note: "Company page token. Expires — the health check is what catches it.",
  },
  outreach: {
...
export async function getSiteContext(slug) {
  ...
  return { site, creds, health, db, sections: normaliseSections(site.sections) };
}
