...
/**
 * The instant the current UK day began, as a real Date.
 *
 * Lives here because this file already owns the UK-wall-clock helpers, and
 * "today" has to mean the same thing to the spend cap as it does to the
 * publishing calendar. The servers run in UTC, so through BST a naive
 * midnight-UTC boundary would start the day an hour late and hand back an
 * hour of yesterday's spend as today's.
 */
export function ukDayStart(from = new Date()) {
  const day = ukDayParts(from, 0);
  const naive = new Date(
    Date.UTC(day.getUTCFullYear(), day.getUTCMonth(), day.getUTCDate(), 0, 0, 0, 0)
  );
  return new Date(naive.getTime() - ukOffsetMinutes(naive) * 60000);
}
