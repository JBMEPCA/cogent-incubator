...
import { houseStyle, siteHost, userAgent } from "./voice";
...
export async function draftArticle(site, articleId, wp) {
  ...
  recordUsage(response.model || "claude-opus-4-8", response.usage);
  ...
  try {
    ...
    image = await chooseSmartImage(site, { title, keyphrase, category });
  } catch {}
  ...
  try {
    ...
    qa = await reviewArticle({ site, title, body, type: article.type, keyphrase, metaDesc });
  } catch (e) {
...
${article.body}`,
  ...
  try {
    const { reviewArticle } = await import("./qa");
    qa = await reviewArticle({ site, title: article.title, body, type: article.type, keyphrase: article.keyphrase, metaDesc });
  } catch (e) {
    qa = { ok: false, issues: [`QA error: ${e.message}`], report: null, score: 0 };
  }
