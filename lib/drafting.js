...
/**
 * A press release that arrived at press@<title>.
 *
 * Differs from the wire rewrite in three ways that matter. The full release is
 * in hand, so there is nothing to excuse thinness with. The same text went to
 * dozens of outlets, so a copied sentence is worth nothing in search and a
 * fresh structure is the whole point. And quotes are the one thing that must
 * NOT be rewritten: a spokesperson's words changed by us are words put in their
 * mouth, which is exactly what the SEO agent did to Karl Foster in September.
 */
function buildPressPrompt(site, article, sourceText, brandName) {
  const host = siteHost(site) || "this site";
  const readers = site?.audience || "the reader";
  const who = brandName || "the company";
  return `Write a news article for ${site?.name || "this publication"} (350-550 words) from the press release below, which ${who} sent to our press desk.

Structure:
- HEADLINE: 60 characters or fewer. Who did what, with a number if the release has one. No colon-and-subtitle padding.
- STANDFIRST: one sentence holding the whole story.
- The first body sentence repeats the standfirst almost verbatim. That is deliberate.
- The second sentence is concrete: what it is, where, what it costs, when it starts.
- Paragraphs of two to four sentences.

Rules:
- Write it fresh. The same release went to dozens of other outlets, so do not copy
  its sentences or its order. Lead with why this matters to ${readers}.
- Strip the sales language: no "delighted to announce", "leading provider",
  "innovative", "cutting-edge", "world-class". Attribute claims ("the company says").
- QUOTES ARE SACRED. Any words you put inside quotation marks must be copied from the
  release character for character, and attributed to the person the release names.
  You may shorten a quote only by quoting a smaller contiguous piece of it. Never
  paraphrase inside quotation marks, never join two quotes, never invent a quote,
  commentator, analyst or statistic. Use at most two quotes.
- Every number, name, date and price you state must appear in the release. If you are
  not sure it is in the release, leave it out.
- ${article.sourceUrl ? `Link ${who} to <a href="${article.sourceUrl}">its own website</a> on first mention.` : `Name ${who} clearly; do not invent a URL for it.`}
- Weave 2 to 4 internal links to existing ${host} articles from the list you were given, with descriptive anchor text, only where genuinely relevant.
- Leave out the "Notes to editors", "About ${who}" boilerplate, media contact details and any embargo line.
- Close with a short practical takeaway for ${readers}.

${untrustedBlock(sourceText || "", "PRESS RELEASE, as received by email")}`;
}

function buildPrompt(site, article, sourceText, brandName, { pressRelease = false } = {}) {
  const host = siteHost(site) || "this site";
  const readers = site?.audience || "the reader";
  if (article.type === "pr_rewrite" && pressRelease) return buildPressPrompt(site, article, sourceText, brandName);
...
/**
 * opts, all optional, exist for the press@ intake (lib/press-intake.js):
 *   sourceText     the release itself, so nothing is fetched from a URL
 *   pressRelease   use the press-release prompt (quotes verbatim, no invention)
 *   image          a picture already chosen from the email; skips every search
 *   effort         output_config effort for the draft. A 500-word rewrite of a
 *                  release in hand needs little deliberation, and at the default
 *                  Sonnet spent 2,700 thinking tokens on one: two thirds of the
 *                  draft's cost, measured 21 Sep 2026.
 *   skipQa         leave the gate to the caller, which runs its own free checks
 *                  first so a fault costs one repair and ONE gate, not two gates
 *   status         what to leave the article as. The intake passes "idea" so
 *                  fill-schedule, publish-due and the Designer never touch it:
 *                  it publishes the piece itself, within the hour.
 */
export async function draftArticle(site, articleId, wp, opts = {}) {
  ...
  const response = await client.messages.stream({
    ...
    messages: [
      ...
      { role: "user", content: buildPrompt(site, article, sourceText, brandName, { pressRelease: opts.pressRelease }) },
    ],
  }).finalMessage();
  ...
  // A story about a person leads with that person: their photo, or a name card
  // while lib/headshots.js asks the company for one. Never stock. JB's rule of
  // 22 Sep 2026, after three Golf appointment stories ran under fairways.
  //
  // The subject is classified on every news piece, even one that already has
  // the sender's own picture from the press desk, because it is stored and the
  // headshot request reads it later. A guide has no subject, so it is skipped.
  let subject = null;
  if (article.type !== "seo_original") {
    try {
      const { pictureForPerson, classifySubject } = await import("./person-image");
      if (image) {
        subject = await classifySubject(title, body);
      } else {
        const upload = wp && wp.url
          ? async (data, contentType, filename) => {
              const { uploadMedia } = await import("./wordpress");
              return uploadMedia(wp, { data, contentType, filename });
            }
          : null;
        const got = await pictureForPerson(site, { title, body, sourceUrl: article.sourceUrl, upload });
        subject = got.subject;
        if (got.image) {
          image = got.image;
          console.log(`[images] ${site.slug}: person story, ${got.image.source} for "${title.slice(0, 60)}"`);
        }
      }
    } catch (e) {
      imageNote = `person check errored: ${e.message?.slice(0, 120)}`;
    }
  }
  ...
  //
  // The subject classified above is passed in, so this does not ask again.
  if (!image && article.type === "pr_rewrite" && article.sourceUrl) {
    try {
  ...
  await db.article.update({
    ...
    data: {
      ...
      ...(subject ? (await import("./person-image")).subjectFields(subject) : {}),
      category,
      keyphrase,
      metaDesc,
      qaPassed: qa.ok,
 ...
 */
export async function repairArticle(site, articleId, wp, opts = {}) {
  const db = forSite(site.id);
  if (!wp) wp = await wordpressCredential(site.id);
...
${article.body}`,
  ...
  try {
    const { reviewArticle } = await import("./qa");
    qa = await reviewArticle({ site, title, body, type: article.type, keyphrase: article.keyphrase, metaDesc, brief: article.brief, effort: opts.gateEffort });
  } catch (e) {
    qa = { ok: false, issues: [`QA error: ${e.message}`], report: null, score: 0 };
  }
  ...
  await db.article.update({
    ...
    data: {
      ...
      metaDesc,
      ...(newTitle ? { title } : {}),
      status: unmoved ? "idea" : opts.status || "review",
