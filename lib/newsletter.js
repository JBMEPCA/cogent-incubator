...
// How many complaints have to sit behind that rate before it stops a send. A
// rate computed from a single click is not a rate. See lastIssueHealth.
const MIN_COMPLAINTS_TO_BLOCK = 3;
 ...
 */
// The pin is a running order, not just a lead. "1295" pins the lead exactly as
// before; "1295,1310,1277" also pins the two editor's picks; a blank position
// ("1295,,1277") is left to the agent. Every position the editor did not name
// is filled from the agent's choice, in the agent's order.
//
// Added 16 September 2026, when JB reviewed the proofs and moved the picks
// round on all four titles. The agent re-ranks from scratch at send time, so a
// lead-only pin would have sent a different issue from the one he approved.
export const parsePins = (value) =>
  String(value ?? "")
    .split(",")
    .map((s) => s.trim())
    .slice(0, STORY_COUNT);

function applyLeadPin(stories, candidates, pinValue) {
  const ids = parsePins(pinValue);
  if (!ids.some(Boolean)) return { stories, pinned: false };
  const byId = new Map(candidates.map((c) => [String(c.id), c]));
  for (const s of stories) byId.set(String(s.id), s);

  // A pin for an article that cannot be found is ignored, not fatal: a missing
  // override should cost the running order, never the issue.
  const missing = ids.filter((id) => id && !byId.has(id));
  const pinnedIds = new Set(ids.filter((id) => id && byId.has(id)));
  const fill = stories.filter((s) => !pinnedIds.has(String(s.id)));

  const out = [];
  for (let i = 0; i < STORY_COUNT; i++) {
    const id = ids[i];
    const next = id && byId.has(id) ? byId.get(id) : fill.shift();
    if (next) out.push(next);
  }
  return {
    stories: out,
    pinned: Boolean(ids[0] && byId.has(ids[0])),
 ...
 *
 * The file name carries a hash of the image. With a fixed name, a title whose
 * logo changed kept finding its OLD file and never uploaded the new one, so a
 * redesign could never reach the newsletter (September 2026 mastheads).
 */
function logoName(site, pngBase64) {
  const hash = crypto.createHash("sha1").update(String(pngBase64 || "")).digest("hex").slice(0, 10);
  return `${site?.slug || "title"}-wordmark-${hash}.png`;
}

async function findLogo(LOGO_NAME) {
  const PAGE = 1000;
...
export async function ensureLogo(site, pngBase64) {
  // No mark means no masthead, never another title's.
  if (!pngBase64) return "";
  const LOGO_NAME = logoName(site, pngBase64);
  const hit = await findLogo(LOGO_NAME);
  if (hit) return hit;
...
/**
 * The account's recent campaign reports, fetched once per run.
 *
 * This list is the same for every title — it is the whole ACCOUNT's history,
 * filtered by list_id at each call site — and it costs about twenty seconds to
 * pull. The weekly job asked for it twice per title, so a fleet run spent most
 * of its wall clock re-downloading identical data: three titles took 202s of a
 * 300s ceiling, and five titles would not have finished at all. A run that
 * exceeds maxDuration does not fail loudly; the titles it never reached simply
 * get no issue that week.
 *
 * Invalidated immediately after any send, so the repeat-send guard below can
 * never read a cache that predates the issue it is checking for.
 */
const REPORTS_TTL_MS = 5 * 60 * 1000;
let reportsCache = null;

// The five-minute cache only ever worked on a long-lived server.
//
// A module-level variable lives in ONE serverless instance, and Vercel spreads
// requests across instances that start cold, so in practice almost every read
// paid the full 200-report scan - the single slowest call in the app, and the
// reason the Engine Room took fifteen seconds to open. unstable_cache is shared
// across invocations, which is what this was always meant to be.
//
// The in-memory copy is kept in front of it: within one request, several
// callers ask for this, and there is no point going even to the shared cache
// four times. invalidateReportsCache() still clears the local copy immediately
// after a send, and the shared TTL is short enough that the repeat-send guard
// reads /campaigns directly rather than trusting this at all.
async function recentReports() {
  if (reportsCache && Date.now() - reportsCache.at < REPORTS_TTL_MS) return reportsCache.reports;
  const reports = await unstable_cache(
    async () => {
      const d = await mc(`/reports?count=200&type=regular&sort_field=send_time&sort_dir=DESC`);
      return d.reports ?? [];
    },
    ["mailchimp-recent-reports"],
    { revalidate: 300 }
  )();
  reportsCache = { at: Date.now(), reports };
  return reports;
}
...
// The Monday briefing and the Thursday weekly are two products on one list,
// told apart by campaign title. Each guard only counts its own kind: a Monday
// 14:00 send is 67 hours before Thursday 09:05, inside the 72-hour window, so
// a shared guard would have cancelled every Thursday issue.
export const BRIEFING_TITLE_MARK = "Monday Briefing";
const isBriefingTitle = (t) => String(t || "").includes(BRIEFING_TITLE_MARK);

/** When this audience last received an issue of this kind, or null. */
export async function lastIssueSentAt(audienceId, { kind = "weekly" } = {}) {
  const mine = (title) => (kind === "briefing" ? isBriefingTitle(title) : !isBriefingTitle(title));
  // /reports only lists campaigns that have FINISHED sending, and it is cached
  // on top of that. A campaign still going out is invisible to it.
  //
  // On 27 August 2026 that sent The Fleet Magazine's first ever issue twice, to
  // the same 1,000 people, two minutes apart: the scheduled run was mid-send, a
  // second run asked this guard whether anything had gone recently, and reports
  // truthfully said no. The audience is the thing being protected, so the
  // question has to be "is one in flight OR just gone", not "has one landed".
  //
  // /campaigns shows a campaign the moment it is created, whatever its status,
  // and is read uncached for exactly that reason.
  const [reports, live] = await Promise.all([
    recentReports(),
    mc(`/campaigns?list_id=${audienceId}&count=5&sort_field=create_time&sort_dir=DESC`).catch(() => null),
  ]);

  const times = [];
  const finished = reports.find((x) => x.list_id === audienceId && x.send_time && mine(x.campaign_title));
  if (finished) times.push(new Date(finished.send_time).getTime());

  // save and schedule count too: a draft this engine created moments ago is
  // about to become a send, and a second run must not start another.
  const IN_FLIGHT = ["save", "schedule", "scheduled", "sending", "sent"];
  for (const c of live?.campaigns ?? []) {
    if (!IN_FLIGHT.includes(c.status)) continue;
    if (!mine(c.settings?.title)) continue;
    const when = c.send_time || c.create_time;
    if (when) times.push(new Date(when).getTime());
  }
...
export async function lastIssueHealth(audienceId) {
  ...
  const mine = reports.filter((x) => x.list_id === audienceId && x.emails_sent);
  const r = mine[0];
  if (!r) return { ok: true, first: true };

  const hard = r.bounces?.hard_bounces ?? 0;
  const soft = r.bounces?.soft_bounces ?? 0;
  const complaints = r.abuse_reports ?? 0;
  // HARD bounces only, JB's decision on 18 September 2026. A soft bounce is a
  // full inbox or a server that was briefly down: the address is real and
  // Mailchimp retries it. Counting those put Fleet (2.84%) and Airport (2.34%)
  // over the line after the lists doubled, and stopped both titles. A hard
  // bounce is an address that does not exist, which is what hurts the sender's
  // reputation, so the 2% ceiling now applies to that alone.
  const bounceRate = hard / r.emails_sent;
  const complaintRate = complaints / r.emails_sent;

  // A dirty list cleans ITSELF in one send, so one bad issue must not lock a
  // title out for ever. Mailchimp moves every hard bounce to "cleaned" as the
  // issue goes out, and a cleaned address is never mailed again: the addresses
  // that caused the rate are gone before the next issue is built. Barbering's
  // first send on 21 September 2026 hard-bounced 66 of 1,000 freshly imported
  // addresses, all 66 were cleaned that day, and on the old rule the title
  // could never send again - the only report it had would fail the gate every
  // Thursday for ever, with nothing a human could do about it.
  //
  // So a single issue over the ceiling is a WARNING, and two consecutive issues
  // over it are a block. Twice means the addresses that fail are not being
  // burned off, which is a list that keeps making bad addresses rather than one
  // dirty import, and that is the thing worth stopping for.
  //
  // Complaints are not softened. Nothing removes a complainer automatically and
  // one issue of it is enough to hurt the sending domain.
  const prev = mine[1];
  const prevBounceRate = prev?.emails_sent ? (prev.bounces?.hard_bounces ?? 0) / prev.emails_sent : 0;
  const repeatedBounce = bounceRate > MAX_BOUNCE_RATE && prevBounceRate > MAX_BOUNCE_RATE;

  const reasons = [];
  if (repeatedBounce)
    reasons.push(
      `bounce rate ${(bounceRate * 100).toFixed(2)}% after ${(prevBounceRate * 100).toFixed(2)}% on the issue before it`
    );
  // Complaints are still not softened the way bounces are, but the rate needs
  // enough behind it to mean anything. One reader hitting "report spam" instead
  // of unsubscribe is 0.108% of a 927-send issue and 0.014% of Smart SME's
  // 6,951, so on the rate alone the SMALLEST title is the one locked out, for
  // the single click that does the least harm. Barbering was blocked exactly
  // that way on 24 September 2026: one complaint, zero hard bounces, 34.3%
  // opens, and no issue it could ever send that would clear the gate.
  //
  // Under the floor it is a warning and the issue goes. Over it the rate blocks
  // as before, because several complaints on one issue is a pattern rather than
  // one annoyed reader.
  const overComplaintRate = complaintRate > MAX_COMPLAINT_RATE;
  if (overComplaintRate && complaints >= MIN_COMPLAINTS_TO_BLOCK)
    reasons.push(`complaint rate ${(complaintRate * 100).toFixed(3)}% across ${complaints} complaints`);

  const warnings = [];
  if (overComplaintRate && complaints < MIN_COMPLAINTS_TO_BLOCK)
    warnings.push(
      `last issue drew ${complaints} complaint${complaints === 1 ? "" : "s"} on ${r.emails_sent} sends (${(complaintRate * 100).toFixed(3)}%), over the ceiling but under the ${MIN_COMPLAINTS_TO_BLOCK}-complaint floor; sending`
    );
  if (bounceRate > MAX_BOUNCE_RATE && !repeatedBounce)
    warnings.push(
      `last issue hard-bounced ${(bounceRate * 100).toFixed(2)}%, those addresses are cleaned off the list; sending, and it blocks if it happens again`
    );
...
export async function newsletterStatus(siteId, mailchimp) {
  ...
  const r = reports.find((x) => x.list_id === audienceId);
  return {
    configured: true,
...
export async function fetchCandidates(wp, limit = 40, site = null, include = null, slugs = null) {
  ...
  // `include` fetches named posts, for a pinned story that has aged out of the
  // newest-forty window between the proof and the send. `slugs` does the same
  // by URL slug, for the Monday briefing's most-read list from GA4.
  const only = include?.length
    ? `&include=${include.map((id) => encodeURIComponent(id)).join(",")}`
    : slugs?.length
      ? `&slug=${slugs.map((s) => encodeURIComponent(s)).join(",")}`
      : "";
  const url = `${base}/wp-json/wp/v2/posts?per_page=${limit}${only}&_embed=wp:featuredmedia,wp:term&orderby=date&order=desc`;
  ...
  return posts
    .map((p) => {
      ...
      return {
        id: p.id,
        // strip, not decode. Interview titles carry a franchise eyebrow span
        // ("<span class="franchise-eyebrow">SME Leaders:</span> ..."), and the
        // title goes straight into the Mailchimp subject line and preview text.
        // decode only unescapes entities, so the subject would have gone out as
        // raw markup. The body was safe because the template strips it again.
        ...
        imageSquare: sizes.thumbnail?.source_url ?? sizes.medium?.source_url ?? media?.source_url ?? "",
      };
    })
...
export async function runNewsletter(
  site,
  { creds, dryRun = false, logoBase64 = null, force = false, testEmails = null } = {}
) {
  ...
  // A warning sends, but it must not send silently: this is what a title looks
  // like one bad issue before it stops.
  for (const w of health.warnings || []) console.warn(`newsletter ${site.slug}: ${w}`);
  ...
  return runAgent(site, "newsletter", "weekly_issue", `Build and send the ${site.name} weekly issue`, async ({ think, progress, say }) => {
    ...
    const pinValue = await getLeadPin(site);
    // Pinned posts outside the newest forty are fetched by id, so a running
    // order approved on Wednesday still sends on Thursday after three more
    // articles have gone live. The agent's menu is left exactly as it was.
    let pool = candidates;
    const outside = parsePins(pinValue).filter((id) => id && !candidates.some((c) => String(c.id) === id));
    if (outside.length) {
      const extra = await fetchCandidates(creds?.wordpress, outside.length, site, outside).catch(() => []);
      pool = [...candidates, ...extra];
    }
    const pin = applyLeadPin(stories, pool, pinValue);
    stories = pin.stories;
    if (pin.pinned) await progress(`lead pinned by editor: ${stories[0].title.slice(0, 60)}`);
    if (pin.pinnedCount > 1) await progress(`${pin.pinnedCount} positions pinned by the editor`);
    ...
    // Drawn width of this title's masthead, scaled by its shape.
    const logoWidth = mastheadDisplayWidth(wordmarkFor(site.slug));

    const sections = renderSections(stories, { campaign: utmCampaign, site });
    ...
    if (dryRun) {
      ...
      const logoUrl =
        (logoBase64 && (await findLogo(logoName(site, logoBase64)))) ||
        (process.env.APP_URL
          ? new URL(`/api/brand/logo/${site.slug}?kind=masthead`, process.env.APP_URL).toString()
          : "");
      const proofHtml = spliceSections(
        renderShell({ site, issueDate: issueDate(now), logoUrl, logoWidth, campaign: utmCampaign }),
        sections
      );
      return {
        ...
        pinnedCount: pin.pinnedCount || 0,
        pinsMissing: pin.missing || null,
    ...
    // A proof to named humans, not to the audience.
    //
    // This is the only way to see what Mailchimp will actually send: the merge
    // tags in the footer resolve, the template renders through their pipeline,
    // and it arrives in a real client rather than a browser. The draft is
    // deleted afterwards so it cannot be mistaken for a scheduled issue, and
    // the lead pin is NOT consumed — nothing went to a subscriber, so the
    // editor's choice still has an issue to lead.
    if (testEmails?.length) {
      await progress(`sending a proof to ${testEmails.join(", ")}`);
      await mc(`/campaigns/${campaign.id}/actions/test`, {
        method: "POST",
        body: JSON.stringify({ test_emails: testEmails, send_type: "html" }),
      });
      await mc(`/campaigns/${campaign.id}`, { method: "DELETE" });
      return {
        summary: `Proof sent to ${testEmails.length} address(es): lead "${lead.title.slice(0, 60)}"`,
        proofSentTo: testEmails,
        stories: stories.map((s) => ({ id: s.id, title: s.title, category: s.category })),
        leadPinned: pin.pinned,
      };
    }
    ...
    // An issue just went out, so every cached answer about "when did this
    // audience last receive one" is now wrong. Clearing here is what makes the
    // cache safe: the repeat-send guard re-reads from Mailchimp rather than
    // from a snapshot taken before this send.
    ...
    if (pin.pinned || pin.pinnedCount) await setLeadPin(site, null);
