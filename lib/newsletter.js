...
const AUDIENCE_ID = process.env.MAILCHIMP_AUDIENCE_ID || "707a3f613c"; // Smart SME
const TEMPLATE_NAME = "Smart SME Weekly";
const LOGO_NAME = "smart-sme-wordmark.png";
...
export function isNewsletterConfigured() {
  return Boolean(process.env.MAILCHIMP_API_KEY && process.env.ANTHROPIC_API_KEY);
}
 ...
 */
async function ensureLogo(pngBase64) {
 ...
 */
async function ensureTemplate(html) {
 ...
 */
export async function lastIssueHealth() {
  ...
  const r = (reports.reports ?? []).find((x) => x.list_id === AUDIENCE_ID);
 ...
 */
export async function newsletterStatus(siteId) {
  const db = forSite(siteId);
  if (!isNewsletterConfigured()) return { configured: false };

  const [list, reports] = await Promise.all([
    mc(`/lists/${AUDIENCE_ID}?fields=name,stats.member_count,marketing_permissions,double_optin`),
...
export async function fetchCandidates(limit = 40) {
  const base = process.env.WP_URL?.replace(/\/$/, "") || "https://smartsme.co.uk";
  ...
  const res = await fetch(url, { headers: { "User-Agent": "SmartSME-Newsletter/1.0" } });
...
const SYSTEM = `You are the Newsletter Manager for Smart SME, a UK publication for owner-managers of small businesses, mostly under £1m turnover.

Each week you choose which ten articles go into the Smart SME Weekly email and in what order.
 ...
 */
async function chooseIssue(think, progress, candidates) {
...
async function checkLinks(stories) {
  ...
  await Promise.all(
    stories.map(async (s) => {
      try {
        const r = await fetch(s.link, { method: "HEAD", headers: { "User-Agent": "SmartSME-Newsletter/1.0" } });
 ...
 */
export async function runNewsletter({ dryRun = false, logoBase64 = null } = {}) {
  if (!isNewsletterEnabled()) return { skipped: "NEWSLETTER_ENABLED=false" };
  if (!isNewsletterConfigured()) return { skipped: "needs MAILCHIMP_API_KEY and ANTHROPIC_API_KEY" };

  const from = process.env.NEWSLETTER_FROM_EMAIL || "news@news.smartsme.co.uk";
  ...
  return runAgent("newsletter", "weekly_issue", "Build and send Smart SME Weekly", async ({ think, progress, say }) => {
    const candidates = await fetchCandidates(40);
    if (candidates.length < STORY_COUNT) {
      throw new Error(`only ${candidates.length} articles with images available, need ${STORY_COUNT}`);
    }

    const { stories, leadReason, source } = await chooseIssue(think, progress, candidates);
    ...
    const logoUrl = await ensureLogo(logoBase64);
    const templateId = await ensureTemplate(renderShell({ issueDate: issueDate(new Date()), logoUrl }));
    ...
    const campaign = await mc("/campaigns", {
      ...
      body: JSON.stringify({
        ...
        settings: {
          ...
          title: `Smart SME Weekly - ${issueDate(new Date())}`,
          from_name: "James Burke | Smart SME",
