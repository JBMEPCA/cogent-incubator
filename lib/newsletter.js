...
export async function lastIssueSentAt(audienceId) {
  ...
  const times = [];
  const finished = reports.find((x) => x.list_id === audienceId && x.send_time);
...
export async function fetchCandidates(wp, limit = 40, site = null, include = null) {
  ...
  // newest-forty window between the proof and the send.
  const only = include?.length ? `&include=${include.map((id) => encodeURIComponent(id)).join(",")}` : "";
