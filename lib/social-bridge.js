...
export async function bridgeWebhook() {
  ...
  return url && /^https:\/\/hook\.[a-z0-9.-]*make\.com\//i.test(url) ? url : null;
}
...
export async function bridgeFor(site) {
  ...
  try {
    return { linkedin: false, instagram: false, ...JSON.parse(row?.value || "{}") };
  } catch {
    return { linkedin: false, instagram: false };
  }
}
