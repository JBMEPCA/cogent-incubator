...
export async function coverageCheck(site, title, { excludeId } = {}) {
...
export async function alreadyCovered(site, title, opts) {
  const db = forSite(site.id);
  return (await coverageCheck(site, title, opts)).certain;
}
