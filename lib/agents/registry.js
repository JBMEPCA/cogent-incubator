...
export const AGENTS = {
  ...
  researcher: {
    ...
    name: "Researcher",
    role: "Research",
    // Proposing topics is triage: it reads Search Console, the wire and
    // autocomplete and returns a scored shortlist. Nothing it writes is
    // published — the Director picks from the list and the Editor writes the
    // article. £2.83 over a fortnight on the writing model for work whose
    // entire output is a list of candidate headlines.
    model: "claude-sonnet-5",
    goal: "Find the topics and questions this title's readers are actually searching for, before competitors cover them.",
    accent: "#0891b2",
    seat: { x: 18, y: 42 },
  ...
  backlink: {
    ...
    role: "Outreach",
    // Scanning an article for the brands it names, then filling in a fixed
    // email template. Every draft waits for approval before it is sent, so the
    // model is not the last check on anything.
    model: "claude-sonnet-5",
  ...
  newsletter: {
    ...
    role: "Email",
    goal: "Put the ten articles most worth a busy reader's time into the weekly email, in the right order.",
 ...
 */
/**
 * Price lookup that survives a model id it has not seen.
 *
 * The API returns the id it actually served, which can carry a suffix the
 * PRICING keys do not. An exact-match lookup then missed and fell through to
 * OPUS pricing — 5x on both input and output for every Haiku routing call — and
 * did it silently, corrupting the one metric the Finance Manager exists to
 * report. Prefix match first, and say so out loud on a genuine miss rather than
 * quietly assuming the most expensive tier.
 */
function priceFor(model) {
  if (PRICING[model]) return PRICING[model];
  const prefix = Object.keys(PRICING).find((k) => String(model || "").startsWith(k));
  if (prefix) return PRICING[prefix];
  console.warn(
    `[costs] no pricing for model "${model}" — billing it at Opus rates. Add it to PRICING in lib/agents/registry.js.`
  );
