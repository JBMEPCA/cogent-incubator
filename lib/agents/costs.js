...
async function setting(key, fallback) {
  const row = await prisma.engineSetting.findUnique({ where: { key } });
  const n = Number(row?.value);
  return Number.isFinite(n) && n > 0 ? n : fallback;
}

export const getTargetGbp = () => setting(TARGET_KEY, DEFAULT_TARGET_GBP);
export const getRate = () => setting(RATE_KEY, DEFAULT_USD_TO_GBP);
...
export const DEFAULT_FIXED_COSTS = [
  ...
  { key: "domain", label: "Domain (smartsme.co.uk)", monthlyUsd: 1.25, note: "~£12/year at GoDaddy." },
...
export async function getFixedCosts() {
  const row = await prisma.engineSetting.findUnique({ where: { key: FIXED_COSTS_KEY } });
...
export async function saveFixedCosts(items) {
  await prisma.engineSetting.upsert({
    where: { key: FIXED_COSTS_KEY },
...
export async function buildCostReport() {
  ...
  const [byAgentRaw, agg7, agg1, published30, failedRuns, firstRun, fixed] = await Promise.all([
    ...
    prisma.agentRun.aggregate({ where: { startedAt: { gte: d7 } }, _sum: { costUsd: true }, _count: true }),
    prisma.agentRun.aggregate({ where: { startedAt: { gte: d1 } }, _sum: { costUsd: true }, _count: true }),
    prisma.article.count({ where: { publishedAt: { gte: d30 } } }),
    prisma.agentRun.findMany({ where: { startedAt: { gte: d30 }, ok: false }, select: { agentKey: true } }),
    prisma.agentRun.findFirst({ orderBy: { startedAt: "asc" }, select: { startedAt: true } }),
    getFixedCosts(),
  ]);
  ...
  const scripted = await prisma.article.aggregate({
  ...
  const [recentRuns, recentScripted] = await Promise.all([
    prisma.agentRun.findMany({ where: { startedAt: { gte: since } }, select: { startedAt: true, costUsd: true } }),
  ...
  const [targetGbp, rate] = await Promise.all([getTargetGbp(), getRate()]);
  ...
  return {
    ...
    runs24h: agg1._count,
    published30,
    perArticle: published30 ? spend30 / published30 : null,
    ...
    perArticleAgent:
      published30 - scriptedCount > 0 ? agentSpend30 / (published30 - scriptedCount) : null,
 ...
 */
export async function spendWindow(days = 7) {
  const since = new Date(Date.now() - days * 864e5);

  const [agent, measured, unmeasured] = await Promise.all([
    prisma.agentRun.aggregate({ where: { startedAt: { gte: since } }, _sum: { costUsd: true }, _count: true }),
    prisma.article.aggregate({
      where: { publishedAt: { gte: since }, costUsd: { not: null } },
      _sum: { costUsd: true },
      _count: true,
    }),
    prisma.article.count({ where: { publishedAt: { gte: since }, costUsd: null } }),
  ]);
...
export async function getStoredReport() {
  const row = await prisma.engineSetting.findUnique({ where: { key: FINANCE_REPORT_KEY } });
  if (!row) return null;
  try {
