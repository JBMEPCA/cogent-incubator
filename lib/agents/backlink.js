...
export async function runBacklink(trigger = "daily_sweep") {
  return runAgent("backlink", trigger, "Chasing links for new coverage", async ({ progress, say }) => {
    const {
      runBacklinkOutreach,
      runBacklinkCheck,
      runReplyCheck,
    ...
    const me = await prisma.agent.findUnique({ where: { key: "backlink" }, select: { lastRunAt: true } });
    const since = me?.lastRunAt || new Date(Date.now() - 86400000);
    const sentSince = await prisma.outreachEmail.findMany({
