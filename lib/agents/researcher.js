...
const UA = { "user-agent": "Mozilla/5.0 (compatible; SmartSMEResearch/1.0)" };
 ...
 */
async function smallFirmCandidates() {
...
export async function proposeSmallFirmStories(think, say) {
  const items = await smallFirmCandidates();
  if (items.length < 3) return { saved: 0, why: `only ${items.length} candidate items` };

  const raw = await think({
    maxTokens: 1200,
    system: `You are the commissioning editor of Smart SME, a UK publication read by owners of small and medium businesses.
...
[{"item":<the number of the item in the list>,"firm":"the company name","title":"a headline for OUR article","angle":"one sentence on what an SME reader takes away","score":<0-100>}]`,
  ...
  for (const p of picks.slice(0, SMALL_FIRM_PICKS)) {
    ...
    if (await prisma.researchTopic.findFirst({ where: { title: p.title } })) continue;
...
async function gscOpportunities() {
  if (!isGoogleConfigured() || !process.env.GSC_SITE_URL) return [];
  try {
    ...
    const data = await googlePost(
      token,
      `https://searchconsole.googleapis.com/webmasters/v3/sites/${encodeURIComponent(process.env.GSC_SITE_URL)}/searchAnalytics/query`,
...
async function existingTitles() {
  const arts = await prisma.article.findMany({ select: { title: true }, take: 200 });
  return arts.map((a) => a.title);
}

function alreadyCovered(topic, titles) {
...
export async function runResearcher(trigger = "tick") {
  return runAgent("researcher", trigger, "Finding topics worth covering", async ({ think, progress, say }) => {
    await progress("Reading Search Console for near-miss queries");
    const gsc = await gscOpportunities();
    ...
    const wirePool = await prisma.feedItem.findMany({
    ...
    const raw = await think({
      maxTokens: 12000,
      system: `You are the research editor of Smart SME Magazine, read by UK small and medium business owners adopting AI, software and automation.

YOUR STANDING BRIEF is to keep the site supplied with BOTH kinds of article, because it currently publishes only one. A survey of the UK SME market in August 2026 found Smart SME running 0% brand news against 85% at Business Matters and 60% at SME Business News, and appearing on no UK SME media list. The evergreen library is genuinely better than anything the competition has and must keep growing. But a site with only evergreen guides gives a reader no reason to return tomorrow, and gives a PR agency no reason to send us anything, which is why the wire below is thinner than it should be. Wire items are first-class candidates, not a tiebreak.
...
Breadth is wanted. Smart SME is read by every kind of UK small business, not
only technology firms, so range across sectors and across the practical concerns
of running a company: finance, tax, hiring, marketing, operations, compliance,
premises, selling online. A topic does not have to mention AI to belong here.
...
${titles.map((t) => `- ${t}`).join("\n")}`,
    ...
    for (const p of picks) {
      if (!p?.title || alreadyCovered(p.title, titles)) continue;
      const dupe = await prisma.researchTopic.findFirst({ where: { title: p.title } });
      if (dupe) continue;
      ...
      const queued = await prisma.researchTopic.findMany({
    ...
    try {
      ...
      const r = await proposeSmallFirmStories(think, say);
      smallFirm = r.saved;
      smallFirmWhy = r.why;
    } catch (e) {
