...
import { clusterNearMisses, describeClusters, dropSpentSpikes } from "../keyword-clusters";
import { getGoogleAccessToken, googlePost, isGoogleConfigured } from "../google";
import { runAgent } from "./runtime";
import { titleBrief, userAgent, editorialStandard, untrustedBlock } from "../voice";
// Static, not destructured from the dynamic import below: the autocomplete
// seeds need it well before that import runs, and a `const` from a later
// statement is in the temporal dead zone until then.
import { sectionNames } from "../sections";
import { isColdStart } from "../site";
import { recordCandidates, validateCandidates, spokenFor, claimTerm } from "../keyword-registry";
...
You are the commissioning editor. This lane's whole risk is promoting a chamber
notice into a case study, which the tier rules above forbid outright: a case
study needs a real, publicly reported company situation, and no quota can ask
you to invent one.
...
[{"item":<the number of the item in the list>,"firm":"the company name","title":"a headline for OUR article","angle":"one sentence on what an SME reader takes away","score":<0-100>}]`,
    user: untrustedBlock(
      items
        ...
        .join("\n"),
      "CHAMBER AND TRADE BODY FEED ITEMS"
    ),
  });
...
async function gscOpportunities(ga, { days = 93 } = {}) {
...
// The Google edition each market's suggestions come from. This lane was
// hardcoded to hl=en-GB&gl=uk for every title, which quietly UK-locked live
// keyword discovery on the global titles — the wire searches got per-beat
// locales on 24 Aug, but autocomplete never did. Set per run from
// Site.markets, the same way UA is set from the title.
const SUGGEST_EDITIONS = {
  GB: "hl=en-GB&gl=uk",
  US: "hl=en&gl=us",
  AU: "hl=en-AU&gl=au",
  IE: "hl=en-IE&gl=ie",
...
async function autocomplete(seed) {
  ...
  for (const market of MARKETS) {
    const edition = SUGGEST_EDITIONS[market] || SUGGEST_EDITIONS.GB;
    try {
      const res = await fetch(
        `https://suggestqueries.google.com/complete/search?client=firefox&${edition}&q=${encodeURIComponent(seed)}`,
        { headers: UA, signal: AbortSignal.timeout(10000) }
      );
      if (!res.ok) continue;
      const json = await res.json();
      if (Array.isArray(json?.[1])) for (const s of json[1]) out.add(s);
    } catch {
      // One edition down must not cost the others.
    }
  }
  return [...out];
}
...
// Takes the scoped handle. It used to reach for a module-level `db` that has
// not existed since the multi-tenant split — a ReferenceError on every single
// Researcher run, thrown before the first model call, so the topic supply died
// silently and the Director simply ran out of things to commission.
async function existingTitles(db) {
...
/**
 * The wire item a pick refers to, from the W-number the model was asked for.
 *
 * Tolerant deliberately: accepts W12, w12 or 12, and falls back to matching the
 * pick's query text against a wire headline, because the model names the story
 * reliably even when it forgets the reference.
 */
function wireItemFor(pick, wire) {
  if (!pick || String(pick.source || "").toLowerCase() !== "wire") return null;
  const ref = String(pick.wireRef || "").trim();
  const n = Number(ref.replace(/^[Ww]/, ""));
  if (Number.isFinite(n) && n >= 1 && n <= wire.length) return wire[n - 1].id;
  const q = String(pick.query || "").toLowerCase().trim();
  if (q.length > 12) {
    const hit = wire.find((w) => {
      const t = String(w.title).toLowerCase();
      return t.includes(q) || q.includes(t.slice(0, 40));
    });
...
/** Host of a URL, lowercased, without www. Empty string when unparseable. */
function hostOf(u) {
  try {
...
export async function runResearcher(site, trigger = "tick") {
  ...
  MARKETS = Array.isArray(site.markets) && site.markets.length ? site.markets : ["GB"];
  return runAgent(site, "researcher", trigger, "Finding topics worth covering", async ({ think, progress, say }) => {
    ...
    // Cold start: a new domain has no Search Console history for weeks, so the
    // near-miss lane below is empty by definition rather than by failure. Say so
    // in the run log, because "0 near-misses" on title #2 otherwise looks
    // identical to a broken GSC connection on title #1 — and that ambiguity is
    // what `isColdStart` was written for and then never used to resolve.
    const cold = isColdStart(site);
    await progress(
      cold
        ? "Cold start: no Search Console history yet, working from autocomplete and the wire"
        : "Reading Search Console for near-miss queries"
    );
    const gsc = await gscOpportunities(creds.google_analytics);

    // Leaving cold start is a fact about the data, not a decision anyone should
    // have to remember to make in the UI. The moment GSC returns real rows the
    // title has history, so it graduates itself and the near-miss lane starts
    // carrying weight on the next tick.
    if (cold && gsc.length) {
      await db.site.update({ where: { id: site.id }, data: { status: "live" } });
      // say(to, subject, body) - the first argument is the RECIPIENT. This
      // call passed the announcement as the recipient, so Prisma rejected the
      // message for having no subject, the run recorded as failed, and the
      // graduation it was announcing had already happened one line earlier.
      await say("director", `${site.name} is out of cold start`,
        "Search Console is returning real rows, so the near-miss lane carries weight from the next tick.");
    }
    ...
    const nearMiss = gsc
      ...
      .slice(0, 25);
    // The same rows, grouped into the article that would win them, ranked by
    // what the group is worth rather than by any one query inside it. A title
    // with no history yields nothing here, which is correct.
    // A short second window, purely to tell a live term from a spent news
    // cycle. Smart SME earned 3,220 impressions on one restaurant closing over
    // two days in September; on ninety-day totals it dwarfs everything the
    // title has, and commissioning against it would be commissioning against a
    // corpse.
    const recent = await gscOpportunities(creds.google_analytics, { days: 17 });
    const clusters = clusterNearMisses(
      dropSpentSpikes(gsc, recent).filter((r) => r.position >= 4 && r.position <= 40 && r.impressions >= 8),
      { site }
    ).slice(0, 12);
    if (clusters.length) {
      const top = clusters[0];
      await progress(
        `Best search opportunity: "${top.key}", ${top.queries.length} quer${top.queries.length === 1 ? "y" : "ies"}, ` +
          `${top.impressions} impressions at position ${top.position.toFixed(1)}`
      );
    }

    // Every near-miss is a term this title is already visible for, so it goes
    // into the registry whether or not it gets commissioned this week. That is
    // what makes the sweep cumulative rather than starting from nothing on each
    // run, and it is what the cross-title collision check reads.
    await recordCandidates(site, nearMiss.map((r) => ({
      term: r.query,
      source: "gsc",
    ...
    // Bing measures demand on a capped batch, oldest reading first. It cannot
    // tell our readers from the general public, so nothing is SELECTED on this
    // number: it says only whether a term has measurable demand behind it and
    // which way that demand is moving.
    const validated = await validateCandidates(site, { limit: 20 });
    if (validated.checked) await progress(`Measured demand for ${validated.checked} terms`);

    // Terms already spoken for anywhere in the fleet, so the model never
    // proposes them. Filtering afterwards would waste the pick: six topics come
    // back and quietly dropping two leaves four.
    const taken = await spokenFor(site);

    await progress("Pulling autocomplete and People Also Ask phrasing");
    const seeds = nearMiss.slice(0, 4).map((r) => r.query);
    // The fallback seeds a title with no Search Console history yet — which is
    // every new title for its first few weeks. They were hardcoded to Smart
    // SME's own topics, so title #2 would have gone looking for what UK SMEs
    // search for regardless of what it publishes. Derived from the title's own
    // audience and sections instead.
    if (!seeds.length) {
      const readers = String(site.audience || site.name || "").toLowerCase();
      const subjects = sectionNames(site).slice(0, 2).map((s) => s.toLowerCase());
      for (const subject of subjects.length ? subjects : ["software"]) {
        seeds.push(`${subject} ${readers}`.trim().slice(0, 90));
      }
    }
    let questions = [];
    ...
    // WRITEABLE wire only. A Google News link cannot be written from.
    //
    // The link is a JavaScript shell: fetchSourceText returns the two words
    // "Google News", and the RSS summary beside it has a median of 13 words. So
    // a topic proposed from one arrives at the Editor with nothing to cite, and
    // the pre-draft gate parks it. That is the right outcome, but it happens too
    // late: the topic has already taken one of the day's commissions, and the
    // Director only commissions about four a day. On 19 August three of Golf's
    // four commissions were stubs, which is most of why it published nothing.
    //
    // Offering only items with a real article behind them means every commission
    // is one that can actually be written. Google News queries still earn their
    // keep as a discovery signal for which brands to add a direct feed for; they
    // just stop being proposed as stories.
    const writeable = wirePool.filter((i) => hostOf(i.link) !== "news.google.com");
    const perBrand = new Map();
    const wire = [];
    ...
    const titles = await existingTitles(db);

    await progress("Checking which homepage sections are short");
    // sectionNames(site), not a SECTIONS constant: lib/sections.js stopped
    // exporting one when sections became per-title data, so this destructure
    // yielded undefined and the two uses below threw.
    ...
    const raw = await think({
      // 20k, was 12k. Adaptive thinking spends from this same budget and the
      // verdict is written after the thinking - the fourth call site to learn
      // the lesson qa.js, drafting.js and seo-agent.js each learned alone.
      // Fleet's run truncated at 12k on 23 August once GSC data arrived and
      // gave the model more evidence to weigh.
      // 32k, was 20k: Fleet truncated again at 20k on 16 September 2026 with
      // nineteen near-miss queries in the briefing. Only what is used is billed,
      // and think() now streams, so the ceiling costs nothing when unused.
      maxTokens: 32000,
      system: `${titleBrief(site)}
${editorialStandard(site)}
...
Prefer wire items where YOUR readers, as described above, would recognise the
company or could plausibly buy from it. A supplier your readers might actually
use is a story; a company that merely operates in the same economy is not.
...
Choose the 6 best content opportunities from the evidence supplied. Three sources:
- Search Console clusters. The site is ALREADY VISIBLE for these and is not winning them. This is demand you have already earned and are failing to collect, so it is the cheapest traffic available to you. AT LEAST THREE of your six picks must come from this lane whenever it is not empty. Take clusters before singles: a cluster is several searches that one article can answer at once, and the impressions shown are the sum of all of them. Commission ONE article per cluster, never one per query, and say in the title what the whole cluster is about.
- Real question phrasing from autocomplete, which wins featured snippets.
- News pegs from the wire, where one of your readers would actually change a decision.
...
- The one hard test: would one of the readers named at the top of this prompt
  read this to do their job better? If yes it qualifies, whatever the sector.
  Reject it only if it genuinely serves someone else: general consumer advice,
  personal finance wearing a professional hat, or a reader this title does not
  claim. Do NOT reject an item for the size of the organisation involved —
  whether large employers or small ones are in scope is decided by the readers
  line above and by nothing else.
- The wire lane has been taking three quarters of every commission while search
  took four per cent, which is how a title ends up with hundreds of articles and
  no rankings. A wire item must now beat a named search cluster to take its slot,
  not merely be interesting.
...
- Never propose a news topic that is not backed by one of the wire items listed
  ...
  section quota deliberately cannot push for it.
- Titles must be specific and searchable, not clever. Never use em dashes.

Reply ONLY with JSON:
[{"title":"...","category":"one of the sections above","source":"gsc|autocomplete|wire|gap","query":"the search query or wire item this came from","wireRef":"the W-number when source is wire, else null","rationale":"one sentence on why this is worth writing now","score":<0-100 opportunity>}]

WHEN source IS "wire" YOU MUST GIVE THE wireRef: the W-number of the item you
are working from, exactly as listed. It is how the writer gets the story to
cite. Without it the piece is commissioned as an evergreen guide about an event
it cannot source, and the editorial gate refuses it every time.`,
      user: `SEARCH CONSOLE: WHAT THIS TITLE ALREADY RANKS FOR AND IS NOT WINNING
(last 90 days. A CLUSTER is several searches that one article can answer; the
impressions shown are the sum of all of them. Ranked by what each is worth to
this title, best first. Commission one article per cluster, not one per query.)
${
        describeClusters(clusters) ||
        (cold
          ? "(none — this title is new and has no Search Console history yet. That is expected, not a failure. Work from the autocomplete phrasing and the wire below, and do not treat the absence as a signal that nothing is worth writing.)"
...
${untrustedBlock(
  wire.map((w, i) => `- W${i + 1} [${w.brand.name}] ${w.title}`).join("\n") || "(none)",
  "NEWSWIRE, last 14 days"
)}

ALREADY TARGETED. These search terms are spoken for and must not be proposed again. One marked with another title is being chased by a sibling magazine, and two of ours competing over a single results page helps neither:
${taken.length ? taken.map((t) => `- "${t.term}"${t.mine ? "" : ` (${t.siteName})`}`).join("\n") : "(nothing claimed yet)"}
    ...
    for (const p of picks) {
      ...
      const topic = await db.researchTopic.create({
        data: {
          ...
          // Resolve the W-number back to the real wire item so the Director can
          // commission this as news with something to cite, instead of as a guide
          // about an event nothing can source. Null when the ref does not resolve;
          // the pre-draft gate then stops the piece before it costs anything.
          sourceItemId: wireItemFor(p, wire),
        },
      });

      // Claim the term so no later run, and no sibling title, chases it again.
      // A refusal is normal operation and not an error: another magazine got
      // there first, the topic still stands, it simply carries no claim.
      if (p.query) {
        await claimTerm(site, p.query, { topicId: topic.id, source: p.source || "gap" });
      }
      saved++;
    }
