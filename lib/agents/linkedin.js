...
async function permalinks(ids) {
  const wanted = ids.filter(Boolean);
  if (!wanted.length || !process.env.WP_URL) return {};
  try {
    const res = await fetch(
      `${process.env.WP_URL.replace(/\/$/, "")}/wp-json/wp/v2/posts?include=${wanted.join(",")}&per_page=100&_fields=id,link`,
...
export async function runLinkedIn(trigger = "daily_queue") {
  return runAgent("linkedin", trigger, "Writing posts from the best stories", async ({ think, progress, say }) => {
    const queued = await prisma.linkedInPost.count({ where: { status: { in: ["draft", "approved"] } } });
    ...
    const perf = await topPages();
    const perfByUrl = Object.fromEntries(perf.map((p) => [p.url.replace(/\/$/, ""), p]));

    const alreadyPosted = new Set(
      (await prisma.linkedInPost.findMany({ where: { articleId: { not: null } }, select: { articleId: true } })).map(
        (p) => p.articleId
      )
    );
    ...
    const links = await permalinks(candidates.map((c) => c.wpPostId));
    ...
    for (const article of candidates.slice(0, DAILY_TARGET)) {
      ...
      const url = links[article.wpPostId] || `https://smartsme.co.uk/?p=${article.wpPostId}`;

      const raw = await think({
        maxTokens: 6000,
        system: `You write LinkedIn posts for Smart SME Magazine, read by UK small and medium business owners adopting AI, software and automation.
...
${plain}`,
      ...
      if (!/smartsme\.co\.uk/.test(text)) issues.push("no link");
