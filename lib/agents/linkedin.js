...
import { nextPostingSlot } from "../linkedin";
...
${plain}`,
      ...
      await db.linkedInPost.create({
        data: {
          ...
          // Booked here rather than on approval. Posting is automatic, so the
          // slot is what decides when this goes out, and a draft with no slot
          // never posts at all — see dueFilter().
          scheduledFor: await nextPostingSlot(site),
        },
      });
      drafted++;
    }
