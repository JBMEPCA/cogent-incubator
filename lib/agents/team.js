...
export async function sweepHeldArticles() {
  const held = await prisma.article.findMany({
  ...
  for (const a of held) {
    const attempts = await prisma.agentRun.count({
      where: { agentKey: "editor", articleId: a.id },
    });
...
export async function runEditor(trigger = "topic_commissioned") {
  return runAgent("editor", trigger, "Writing the next commissioned article", async ({ progress, say }) => {
    ...
    if (repairing) {
      ...
      try {
        res = await drafting.repairArticle(article.id);
      } catch (e) {
        ...
        res = await drafting.draftArticle(article.id);
      }
    } else {
...
export async function runDesigner(trigger = "draft_ready") {
  return runAgent("designer", trigger, "Sourcing a header image", async ({ progress, say }) => {
    const article = await prisma.article.findFirst({
      where: { status: { in: ["review", "approved"] }, imageUrl: null },
      orderBy: { createdAt: "desc" },
    });
    if (!article) return { summary: "Every article already has an image" };

    await progress(`Finding an image for "${article.title.slice(0, 55)}"`);
    const { chooseSmartImage } = await import("../images");
    const image = await chooseSmartImage({
      title: article.title,
    ...
    if (!image) {
      await say("director", `No usable image for "${article.title.slice(0, 50)}"`,
        "Every candidate failed the visual check. Abstract topics often need explicit art direction.", "conflict");
      return { summary: `No image passed the gate for "${article.title.slice(0, 50)}"`, articleId: article.id };
    }
    await prisma.article.update({
      where: { id: article.id },
...
export async function runSeo(trigger = "weekly_audit") {
  return runAgent("seo", trigger, "Auditing links and search performance", async ({ progress, say }) => {
    const { runSeoAudit, isSeoAgentConfigured, MIN_INTERNAL_LINKS } = await import("../seo-agent");
    if (!isSeoAgentConfigured()) {
      return { summary: "Standing by: the audit needs ANTHROPIC_API_KEY and WordPress credentials" };
    }
    ...
    const noKeyphrase = await prisma.article.count({
...
export async function runFinance(trigger = "daily_summary") {
  return runAgent("finance", trigger, "Costing the operation", async ({ think, progress, say }) => {
...
Cost per published article, all in: ${report.perArticle ? "$" + report.perArticle.toFixed(3) : "nothing published yet"}
...
export async function runDirector(trigger = "tick") {
  return runAgent("director", trigger, "Reviewing the team", async ({ think, progress, say }) => {
    await progress("Reading open reports from the team");
    const open = await prisma.agentMessage.findMany({
      where: { toKey: "director", resolved: false },
      orderBy: { createdAt: "desc" },
      take: 15,
    });

    const proposed = await prisma.researchTopic.findMany({
    ...
    const needsImage = await prisma.article.count({ where: { status: { in: ["review", "approved"] }, imageUrl: null } });
    ...
    const requested = await prisma.researchTopic.findFirst({
      where: { status: "proposed", source: "jb" },
      orderBy: { createdAt: "asc" },
    });
    if (requested) {
      const { alreadyCovered } = await import("./dedupe");
      const clash = await alreadyCovered(requested.title);
      if (clash) {
      ...
      await prisma.researchTopic.update({ where: { id: requested.id }, data: { status: "commissioned" } });
    ...
    if (inFlight < 2 && proposed.length) {
      ...
      for (const t of proposed) {
        const { certain, maybe } = await coverageCheck(t.title);
        if (certain) {
          await prisma.researchTopic.update({
            where: { id: t.id },
      ...
      const verdict = await think({
        maxTokens: 6000,
        system: `You are the Director of Smart SME Magazine, read by UK small and medium business owners. The publication covers anything that helps someone run a small business better: AI and software, but equally finance, tax, hiring, marketing, operations, compliance and selling online, across every sector.

YOUR STANDING BRIEF is to hold the shape of the publication. Smart SME is a reference library that also runs a newsroom, and the newsroom is the half that gives a reader a reason to come back tomorrow and puts us on the media lists PR agencies work from. Left alone that balance drifts towards evergreen every time, because a guide can always be commissioned and news can only be found. In August 2026 the drift had reached 0% news across the fifteen most recent articles while 1,450 collected wire stories sat unread. Catching that drift is your job and nobody else's: no other agent sees the mix.
...
THE HARD LINE, which outranks the target: news must be tied to a real event actually on the wire. You cannot commission news into existence. If nothing on the wire is worth writing, commission the best guide and let the figure sit low. A fabricated or backdated news story is a far worse outcome than a quiet week, and the section quota is deliberately built so it can never ask you for one. The formats and their rules are in docs/editorial-standard.md.

Range is wanted, so judge generously. Only strike a topic out if it genuinely does not belong: consumer advice rather than business advice, content aimed at large corporates, or personal finance wearing a business hat.
...
Reply ONLY with JSON: {"commission": <index>, "why": "one sentence", "reject": [{"index": <n>, "reason": "one sentence"}]}`,
      ...
      for (const r of Array.isArray(ruling.reject) ? ruling.reject : []) {
        ...
        if (!t) continue;
        await prisma.researchTopic.update({ where: { id: t.id }, data: { status: "rejected", rationale: r.reason || t.rationale } });
      ...
      if (pick && pick.status !== "rejected" && !(ruling.reject || []).some((r) => shortlist[r.index]?.id === pick.id)) {
        ...
        const wireItem = chamberPick
          ? pick.query
            ? await prisma.feedItem.findFirst({ where: { link: pick.query } })
            : null
          : pick.source === "wire"
            ? await prisma.feedItem.findFirst({ where: { title: pick.query || pick.title } })
            : null;
        ...
        if (wireItem) {
          await prisma.feedItem.update({ where: { id: wireItem.id }, data: { status: "drafted" } });
        }
        await prisma.researchTopic.update({ where: { id: pick.id }, data: { status: "commissioned" } });
    ...
    if (open.length) {
      ...
      const ruling = await think({
        maxTokens: 6000,
        system: `You are the Director of an AI content team for Smart SME Magazine (UK SMEs adopting AI, software and automation). Your team: Researcher (finds topics), SEO Expert (search performance), Editor (writes, refuses to publish invented facts), Graphic Designer (images), Finance Manager (advisory cost reporting).
...
Reply ONLY with JSON: [{"subject":"...","ruling":"one or two sentences","action":"none|retry|reject|escalate"}]`,
      ...
      for (const r of Array.isArray(rulings) ? rulings : []) {
        ...
        if (!target) continue;
        await prisma.agentMessage.update({ where: { id: target.id }, data: { resolved: true } });
