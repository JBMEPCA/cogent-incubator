...
import { linkArticle } from "../keyword-registry";
...
// One more repair, but only for an article that is nearly there.
//
// The gate passes an article only when it lists ZERO issues, so a piece
// scoring 83 with a single fixable note - "fix the six-year vs seven-year
// contradiction", "attribute the 900-courses figure" - is held exactly like a
// bad one. It then gets ONE repair, and if that fixes the fault while the gate
// notices a different one, the article is parked for good.
//
// Golf Resort Magazine had FIFTEEN articles in that hole on 4 September and
// published five all week: scores of 78, 79, 80, 82, 83, each needing one
// small correction, all abandoned. Airport, whose drafts come back cleaner,
// published eighteen.
//
// So a near-miss earns a third go and a genuinely weak draft still stops at
// two. The cost argument that set this to 2 was measured when a repair was an
// Opus redraft; repair runs on Sonnet now, and a third pass on a piece already
// scoring in the high seventies is the cheapest article on the shelf.
const NEAR_MISS_SCORE = 75;
const MAX_ATTEMPTS_NEAR_MISS = 3;

function attemptsAllowed(qaReport) {
  try {
    const score = JSON.parse(qaReport || "{}").score;
    if (typeof score === "number" && score >= NEAR_MISS_SCORE) return MAX_ATTEMPTS_NEAR_MISS;
  } catch {
    // An unreadable report is not evidence of quality: fall through to strict.
  }
  return MAX_DRAFT_ATTEMPTS;
}
...
export async function sweepHeldArticles(site) {
  ...
  const held = await db.article.findMany({
    ...
    select: { id: true, title: true, qaReport: true },
  });
  ...
  for (const a of held) {
    ...
    const allowed = attemptsAllowed(a.qaReport);
    if (attempts >= allowed) {
      ...
      await say(
        ...
        `QA held it ${attempts} times (budget ${allowed}). Parked as an idea rather than drafted again.`,
        "conflict"
      );
      result.abandoned++;
    } else {
...
//
// Three since 15 September 2026, because running out now RETIRES the article
// instead of leaving it in the queue asking for art direction nobody gives.
const MAX_IMAGE_ATTEMPTS = 3;

// Failed goes at the article's CURRENT headline.
//
// This counted runs since article.updatedAt, meaning to reset only when the
// headline was rewritten. But fill-schedule rewrites scheduledFor on every
// rebalance, which bumps updatedAt, so the count went back to zero every half
// hour: Zoox failed 37 times and Mews 11, each always "on attempt 1/2", each
// taking the picture desk's first turn and, because a batch stops at its first
// failure, blocking every article behind it. On 15 September Mews held Golf's
// only two finished pieces out of the queue after a credit outage.
//
// A failure summary quotes the headline it searched against, so matching on
// that gives exactly the intended rule: the same headline accumulates, a new
// one starts fresh. The Zoox headline rewrite of 14 September found a picture
// on its first go.
const failedAttemptsAt = (runs, article) => {
  const quoted = `"${article.title.slice(0, 40)}`;
  return runs.filter(
    (r) => r.articleId === article.id && /No image on attempt/.test(r.summary || "") && (r.summary || "").includes(quoted)
  ).length;
};
...
export async function imageWorkAvailable(db) {
  ...
  const runs = await db.agentRun.findMany({
    ...
    select: { articleId: true, summary: true },
  });
  return waiting.some((a) => failedAttemptsAt(runs, a) < MAX_IMAGE_ATTEMPTS);
}
...
export async function runDesigner(site, trigger = "draft_ready") {
  ...
  return runAgent(site, "designer", trigger, "Sourcing a header image", async ({ progress, say }) => {
    ...
    const waiting = await db.article.findMany({
      where: { status: { in: ["review", "approved"] }, imageUrl: null },
      // Soonest slot first. This was newest-first, which treats an article due
      // at 07:30 tomorrow exactly like one due on Friday - and on 2 September
      // that cost four titles their 07:30 round: every one of them was still
      // queueing for a picture at the moment publish-due looked, and got its
      // image between 08:06 and 09:06. Postgres sorts NULLs last on ASC, so
      // unslotted work waits behind anything with a time on it, which is right.
      ...
      select: {
        ...
        // A person story never goes to stock: see lib/person-image.js.
        type: true,
        body: true,
        subjectKind: true,
        subjectName: true,
    ...
    // real edit.
    // See failedAttemptsAt: counted per headline, not per last edit.
    const priorRuns = await db.agentRun.findMany({
      where: { agentKey: "designer", articleId: { in: waiting.map((a) => a.id) } },
      select: { articleId: true, summary: true },
    });
    const attemptsOn = (article) => failedAttemptsAt(priorRuns, article);

    // Anything already out of attempts is retired before the desk looks at the
    // rest. The retirement below only fires on a failing run, and an exhausted
    // article is skipped so never fails again: without this sweep it would sit
    // on the shelf holding a slot indefinitely, which is where the pre-fix
    // strays like the 26-attempt waxing guide already are.
    const exhausted = waiting.filter((a) => attemptsOn(a) >= MAX_IMAGE_ATTEMPTS);
    for (const a of exhausted) {
      await db.article.update({ where: { id: a.id }, data: { status: "idea", scheduledFor: null } });
      await say(
        "director",
        `Retired for want of a picture: "${a.title.slice(0, 50)}"`,
        `${attemptsOn(a)} failed picture searches at this headline. Parked as an idea, body kept.`,
        "conflict"
      );
    }
    if (exhausted.length) {
      const gone = new Set(exhausted.map((a) => a.id));
      waiting.splice(0, waiting.length, ...waiting.filter((a) => !gone.has(a.id)));
      if (!waiting.length) return { summary: `Retired ${exhausted.length} article(s) no picture could be found for` };
    }

    // More than one picture per turn, while it keeps succeeding.
    //
    // One per tick was set when the gate ran hot and a run cost real money. It
    // no longer does - 6 runs and 5p on the morning of 2 September against 48
    // runs and 70p on 31 August - and the limit had become the bottleneck
    // instead: five articles needing five pictures took five ticks, which is
    // two and a half hours, which is how a 07:30 round gets missed.
    //
    // The batch stops at the FIRST failure, deliberately. Attempts are counted
    // from AgentRun.articleId, and a run records one article, so a batch that
    // carried on past a failure would lose that article's attempt and retry it
    // for ever. A success needs no counting: the article has its picture and
    // leaves the queue.
    const BATCH = 4;
    const cleared = [];
    let failure = null;
    ...
    // One article's turn, lifted out so the loop below can take several.
    const imageOne = async (article, attempt) => {

      await progress(`Finding an image for "${article.title.slice(0, 55)}"`);
      const { chooseSmartImage, sourceImage } = await import("../images");

      // A story about a person leads with that person, or with a name card
      // while the company is asked for a photo. It never reaches the source
      // og:image or the stock search below, both of which gave Golf's
      // appointment stories a fairway or a company banner.
      if (article.type !== "seo_original") {
        const { pictureForPerson, subjectFields } = await import("../person-image");
        const known = article.subjectKind
          ? { kind: article.subjectKind, name: article.subjectName, role: article.subjectRole, org: article.subjectOrg }
          : null;
        const { siteCredentials } = await import("../site");
        const { creds } = await siteCredentials(site.id);
        const wp = creds?.wordpress;
        const upload = wp?.url
          ? async (data, contentType, filename) => {
              const { uploadMedia } = await import("../wordpress");
              return uploadMedia(wp, { data, contentType, filename });
            }
          : null;
        const got = await pictureForPerson(site, {
          title: article.title,
          body: article.body,
          sourceUrl: article.sourceUrl || article.sourceItem?.link || null,
          subject: known,
          upload,
        }).catch(() => null);
        if (got?.subject && !known) {
          await db.article.update({ where: { id: article.id }, data: subjectFields(got.subject) });
        }
        if (got?.image) {
          await db.article.update({
            where: { id: article.id },
            data: {
              imageUrl: got.image.url,
              imageAlt: got.image.alt,
              imageCredit: got.image.credit,
              imageSource: got.image.source,
            },
          });
          return {
            summary: `${got.image.source === "card:person" ? "Name card" : "Photo of the person"} for "${article.title.slice(0, 50)}"`,
            articleId: article.id,
          };
        }
      }
      ...
      if (!image) {
        ...
        if (lastGo) {
          // Retired, the same way the Editor parks a draft it cannot pass: as an
          // idea, body kept, off the shelf. Leaving it in the queue asked for art
          // direction that never came in a fully automated engine, and meanwhile
          // it held a slot and the desk's first turn. It goes in the kill ledger.
          await db.article.update({
            where: { id: article.id },
            data: { status: "idea", scheduledFor: null },
          });
          await say(
            "director",
            `Retired for want of a picture after ${attempt + 1} attempts: "${article.title.slice(0, 50)}"`,
            [reason, srcNote, ...(tried || []).map((t) => `- ${t}`)].filter(Boolean).join("\n") +
              "\n\nParked as an idea, body kept. A rewritten headline and a return to review gives it a fresh set of attempts.",
            "conflict"
          );
        }
        return {
    ...
    // Work down the queue, soonest slot first, stopping at the first miss.
    for (const a of waiting) {
      if (cleared.length >= BATCH) break;
      const attempt = attemptsOn(a);
      if (attempt >= MAX_IMAGE_ATTEMPTS) continue;
      const r = await imageOne(a, attempt);
      if (r.failed) { failure = r; break; }
      cleared.push(r.summary);
    }

    if (failure) {
      return {
        summary: cleared.length
          ? `${cleared.length} image(s) attached, then: ${failure.summary}`
          : failure.summary,
        articleId: failure.articleId,
      };
    }
    if (!cleared.length) return { summary: `${waiting.length} article(s) waiting on a picture, all past ${MAX_IMAGE_ATTEMPTS} attempts — art direction needed` };
    return {
      summary: cleared.length === 1 ? cleared[0] : `${cleared.length} images attached: ${cleared.join(" | ")}`,
    };
  });
}
...
export async function runDirector(site, trigger = "tick") {
  ...
  return runAgent(site, "director", trigger, "Reviewing the team", async ({ think, progress, say }) => {
    ...
    //
    // Three days since 14 September 2026, at JB's ask to get ahead for the week.
    // Two left no slack for a stuck picture: on 12-13 September titles holding
    // two days of stock still published into empty slots, and Golf opened that
    // Monday with one imaged day in hand. Three is the ceiling, not a step
    // towards more: past it the shelf is news published stale.
    const SHELF_DAYS = 3;
    ...
    if (requested) {
      ...
      const jbArticle = await db.article.create({
        data: {
      ...
      await db.researchTopic.update({ where: { id: requested.id }, data: { status: "commissioned", articleId: jbArticle.id } });
      if (requested.query) await linkArticle(site, requested.query, jbArticle.id);
...
Reply ONLY with JSON: {"commission": <index>, "why": "one sentence", "reject": [{"index": <n>, "reason": "one sentence"}]}`,
      ...
      if (pick && pick.status !== "rejected" && !(ruling.reject || []).some((r) => shortlist[r.index]?.id === pick.id)) {
        ...
        const commissioned = await db.article.create({
        ...
        await db.researchTopic.update({ where: { id: pick.id }, data: { status: "commissioned", articleId: commissioned.id } });
        // Bind the keyword to the article it produced, so the SEO page can answer
        // "what are we doing about this term". Skipped for a chamber pick, where
        // `pick.query` holds the wire item's URL rather than a search term.
        if (!chamberPick && pick.query) await linkArticle(site, pick.query, commissioned.id);
