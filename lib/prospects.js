...
// how many a title takes; this only stops a mistyped allowance turning into a
// hundred Mailchimp calls inside one 300s function.
const MAX_PER_RUN = Number(process.env.DRIP_MAX_PER_RUN || 2000);
...
export async function runDrip(site, { mailchimp, size, force = false } = {}) {
  ...
  // than a fault.
  //
  // A run takes the whole remaining allowance rather than a fixed thousand, so
  // the weekly rate is right whichever schedule is live. That matters because
  // the schedule lives in a Cloudflare Worker deployed separately from this
  // app: if the app ships and the worker does not, Smart SME would otherwise
  // take a thousand on the only run it gets and quietly run at half its rate,
  // which is exactly the kind of half-deployed state that stopped the drip in
  // the first place.
  ...
  const batch = await db.newsletterProspect.findMany({
    where: {
      suppressed: false,
      importedAt: null,
      // Everything except the ones a verifier already condemned.
  ...
  if (!batch.length) {
    ...
    return { imported: 0, message: waiting ? `nothing importable, ${waiting} left unimportation` : "list exhausted" };
  }
