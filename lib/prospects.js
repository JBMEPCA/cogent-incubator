 ...
 */
export async function verifyProspects(limit = VERIFY_BATCH) {
  let credits;
  try {
  ...
  const due = await prisma.newsletterProspect.findMany({
    ...
    Array.from({ length: VERIFY_CONCURRENCY }, async () => {
      while (queue.length) {
        ...
        try {
          ...
          await prisma.newsletterProspect.update({
            where: { id: p.id },
            data: {
 ...
 */
export async function runDrip({ size = IMPORT_BATCH, force = false } = {}) {
  ...
  const batch = await prisma.newsletterProspect.findMany({
    where: { suppressed: false, importedAt: null, verifyStatus: "good" },
    orderBy: { rank: "asc" },
    take: size,
  });
  if (!batch.length) {
    const waiting = await prisma.newsletterProspect.count({ where: { suppressed: false, verifyStatus: null } });
  ...
  const tranche = (await prisma.newsletterProspect.aggregate({ _max: { tranche: true } }))._max.tranche ?? 0;
  ...
  await prisma.newsletterProspect.updateMany({
...
export async function prospectStats() {
  const [total, suppressed, imported, verified, unverified, byResult] = await Promise.all([
    ...
    prisma.newsletterProspect.count({ where: { verifyStatus: "good", suppressed: false } }),
    prisma.newsletterProspect.count({ where: { verifyStatus: null, suppressed: false } }),
    prisma.newsletterProspect.groupBy({ by: ["verifyResult"], _count: true }),
  ]);

  const readyToImport = await prisma.newsletterProspect.count({
    where: { suppressed: false, importedAt: null, verifyStatus: "good" },
  });
