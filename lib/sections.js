...
// could look half-built while holding thirty-odd articles. TARGET is therefore
// a buffer, not a shelf count: six published leaves three for the section after
// the rest of the page has taken its share.
//
// The quota steers commissioning. It never authorises writing something that
// is not true, and two sections are deliberately outside it — see MANUFACTURED.
import { prisma } from "./prisma";
import { categoryCounts } from "./wordpress";

export const TARGET = 6;

// The six categories exactly as WordPress spells them, which is also how the
// Editor is asked to emit CATEGORY in lib/drafting.js. Anything that does not
// match one of these is a typo somewhere in the chain, not a new section.
export const SECTIONS = ["AI & Automation", "Finance", "Marketing", "News", "Operations", "Case Studies"];

// Sections a quota must never push the team to fill.
//
// News is dated by definition: writing it to hit a number means inventing
// events, and backdating it would breach Google News policy on top. Case
// Studies means a real, publicly reported company situation, so it fills from
// the wire when something real lands and stays empty when nothing has. Both
// still count toward the tally so the picture is honest; they just cannot
// generate a "write three more" instruction.
export const MANUFACTURED = { "News": false, "Case Studies": false };

export function canCommission(section) {
  return MANUFACTURED[section] !== false;
}
 ...
 */
export async function sectionGaps() {
  ...
  return SECTIONS.map((name) => {
    ...
    return {
      ...
      commissionable: canCommission(name),
    };
 ...
 */
export function gapBriefing(gaps) {
