...
export async function publishToWordPress(wp, {
  ...
  metaDesc,
  authorId,
  // With status "future", when WordPress should put it live. The press desk
  // uses this for embargoes, so the site holds the piece rather than a cron.
  dateGmt,
}) {
  ...
  const res = await fetch(`${wpBase(wp)}/posts`, {
    ...
    body: JSON.stringify({
      ...
      content: body,
      status,
      ...(dateGmt ? { date_gmt: dateGmt.replace(/\.\d{3}Z$/, "").replace(/Z$/, "") } : {}),
...
async function fetchPostsPage(wp, perPage, page) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (isBotChallenge(res.status, contentType, body)) {
      // ONE more go, then leave it alone.
      //
      // This retried three times four and eight seconds apart, and that is what
      // turns a transient challenge into a lockout: probing the same host six
      // times in twelve seconds on 9 September took Smart SME from a clean
      // HTTP 200 to a solid 202 on every shape of request, small ones included.
      // The trigger is the rate, not the payload.
      //
      // The sweep runs twice a day against a ladder that ticks every half hour,
      // so deferring costs nothing and the host forgets us in the meantime.
      if (attempt < 2) {
        await new Promise((r) => setTimeout(r, 8000));
        continue;
      }
      throw transient(
        `SiteGround bot protection challenged the WordPress API (${res.status}). Not a code fault: the host is rate-limiting this request, which asks for ${perPage} posts with full content. Publishing is unaffected because it makes far smaller calls.`
      );
    }
...
/**
 * Read a WordPress REST reply as JSON, retrying a bot challenge.
 *
 * fetchPostsPage has done this since 11 August; fetchPost and updatePost did
 * not, and the gap cost the fleet 182 links. SiteGround answers a challenge
 * with HTTP 202 and an HTML page, and 202 passes res.ok, so a bare res.json()
 * threw "Unexpected token '<'" and the SEO sweep recorded a permanent failure
 * against a suggestion whose text it had never once looked at. Every one of
 * those was a good link. They died on 27 and 31 August, on all five titles.
 *
 * The distinction that matters to the caller is not the status code, it is
 * whether the POST changed or the HOST got in the way. A challenge, a 5xx, or
 * a 2xx that is not JSON is the host, so it is thrown as retryable and the
 * sweep leaves the suggestion pending rather than burning it.
 */
...
async function wpJson(makeRequest, what) {
  let lastPreview = "";
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (isBotChallenge(res.status, contentType, body)) {
      ...
      throw transient(`${what}: SiteGround bot protection challenged the API (${res.status}) on all 3 attempts`);
    }

    if (res.ok || res.status >= 500) {
      throw transient(`${what}: host answered ${res.status} (${contentType || "no content-type"}): ${lastPreview}`);
    }

    throw new Error(`${what} failed (${res.status}): ${lastPreview}`);
  }

  throw transient(`${what} failed after 3 attempts: ${lastPreview}`);
}
...
export async function fetchPost(wp, wpPostId) {
  return wpJson(
    () =>
      fetch(`${wpBase(wp)}/posts/${wpPostId}?_fields=id,link,title,content,excerpt,date,status`, {
        headers: { authorization: `Basic ${wpAuth(wp)}`, "user-agent": "CogentBot/1.0" },
      }),
    `WP post ${wpPostId} fetch`
  );
}
...
export async function updatePost(wp, wpPostId, data) {
  return wpJson(
    () =>
      fetch(`${wpBase(wp)}/posts/${wpPostId}`, {
        method: "POST",
        headers: {
          authorization: `Basic ${wpAuth(wp)}`,
          "content-type": "application/json",
...
// A post's stored (raw) content and permalink, for edits that must not round-trip
// through the rendered HTML: the headshot swap rewrites one credit line in place.
export async function fetchPostRaw(wp, wpPostId) {
  const res = await fetch(`${wpBase(wp)}/posts/${wpPostId}?context=edit&_fields=id,link,content,featured_media`, {
    headers: { authorization: `Basic ${wpAuth(wp)}`, "user-agent": "CogentBot/1.0" },
  });
  if (!res.ok) throw new Error(`WP post ${wpPostId} fetch ${res.status}`);
  const p = await res.json();
  return { id: p.id, link: p.link, raw: p.content?.raw ?? "", featuredMedia: p.featured_media || null };
}
