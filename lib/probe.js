...
/** LinkedIn: is the stored company-page token still alive? */
async function probeLinkedIn(li) {
  if (!li?.accessToken) return { ok: false, error: "Not connected. Use Connect on the LinkedIn page to authorise the company page." };
  if (li.expiresAt && new Date(li.expiresAt) < new Date()) {
    return { ok: false, error: `The token expired on ${new Date(li.expiresAt).toLocaleDateString("en-GB")}. Reconnect.` };
  }
  if (!li.organisationUrn) return { ok: false, error: "No organisation URN — the token is not tied to a company page." };

  let res;
  try {
    res = await fetch(`https://api.linkedin.com/rest/organizations/${encodeURIComponent(String(li.organisationUrn).split(":").pop())}`, {
      headers: {
    ...
    return { ok: false, error: `Could not reach LinkedIn: ${e.message}` };
  }
  if (res.status === 401) return { ok: false, error: "LinkedIn rejected the token (401). Reconnect the company page." };
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    return { ok: false, error: `LinkedIn returned ${res.status}. ${body.slice(0, 200)}` };
  }
  const org = await res.json().catch(() => ({}));
  return { ok: true, detail: `Posting as ${org.localizedName || li.organisationUrn}` };
}
