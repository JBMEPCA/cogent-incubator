...
export async function runBriefing(site, { creds, dryRun = false, testEmails = null, force = false } = {}) {
  ...
  // A warning has to reach the caller too. Health warnings were dropped here
  // whenever health.ok was true, so the one state most worth watching - over
  // the complaint ceiling but under the floor that blocks - was the state that
  // said nothing at all.
  const healthWarning = health.ok
    ? health.warnings?.length
      ? health.warnings.join("; ")
      : null
    : `would be BLOCKED: previous issue ${health.reasons.join(", ")}`;
  ...
  return runAgent(site, "newsletter", "monday_briefing", `Build and send ${T.name}`, async ({ think, progress, say }) => {
    ...
    // A pin is approved days before it sends, and the pool is only the newest
    // forty plus last week's most read. A busy title publishes straight past a
    // pinned story in between, and a pin that cannot be found is ignored in
    // SILENCE: the rule picks instead and the issue that goes out is not the one
    // the editor signed off, with nothing anywhere saying so.
    //
    // fetchCandidates takes `include` for exactly this and the weekly issue has
    // always used it; the briefing never did. Measured on 25 September, three
    // days before the send: Barbering's pinned lead sat at 34 of 40 and that
    // title publishes 3.3 articles a day, so it would have been past the window
    // by Monday afternoon.
    const missingPins = [...new Set(pins.filter((id) => id && !byId.has(id)))];
    if (missingPins.length) {
      const extra = await fetchCandidates(creds?.wordpress, missingPins.length, site, missingPins).catch(() => []);
      for (const e of extra) {
        byPath.set(pathOf(e.link), e);
        byId.set(String(e.id), e);
      }
      const stillMissing = missingPins.filter((id) => !byId.has(id));
      // Worth a line in the log either way: recovering a pin says the window is
      // too tight for this title, and failing to says the id is wrong.
      await progress(
        stillMissing.length
          ? `pinned ${stillMissing.join(", ")} could not be fetched and will be ignored`
          : `fetched ${missingPins.length} pinned article(s) that had aged out of the window`
      );
    }
