...
Today is ${briefingDate(new Date())}. Never pick a story whose deadline, closing date or event has already passed: a reader opening it today would be told about something they can no longer act on.
...
Reply with ONLY JSON, no prose and no code fence: {"id": <numeric id>, "reason": "one short sentence"}`,
        ...
        if (!lead) {
          lead = pool[0];
          leadReason = "fallback: newest new article";
        }
      }
    }
    ...
    if (mostRead.length < MOST_READ) {
      ...
      for (const v of views) {
        ...
        if (!p || taken.has(String(p.id)) || !isUsable(p) || !p.imageSquare) continue;
        mostRead.push(p);
    ...
    const tease = (t) => {
      let s = shortHeadline(t).replace(/^.{0,40}?(?:Leaders?|In the Chair)\s*:\s*/i, "");
      s = s.split(/,\s|\s+(?:What|With|But|as|after|and)\s/)[0].trim();
      return s.length > 48 ? s.slice(0, s.lastIndexOf(" ", 48)) : s;
    };
    const list = mostRead.map((p) => tease(p.title));
    const previewText = `Plus last week's most read: ${list.slice(0, -1).join(", ")} and ${list.at(-1)}`.slice(0, 145);
    const subject = `${T.emoji ? `${T.emoji} ` : ""}${shortHeadline(lead.title)}`.slice(0, 145);
