...
export async function runBriefing(site, { creds, dryRun = false, testEmails = null, force = false } = {}) {
  ...
  if (!health.ok && !force) return { skipped: `previous issue unhealthy: ${health.reasons.join(", ")}`, health };
...
Reply with ONLY JSON, no prose and no code fence: {"id": <numeric id>, "reason": "one short sentence"}`,
    ...
    const list = mostRead.map((p) => shortHeadline(p.title));
    ...
    const untagged = [...probe.matchAll(/href="(https?:[^"]+)"/gi)]
      .map((m) => m[1].replace(/&amp;/g, "&"))
