...
// A per-day ceiling as well as a per-run one. The per-run cap alone left the
// theoretical maximum at 10 messages a tick x 14 ticks = 140 a day per title,
// with nothing anywhere counting - a runaway drafting bug would have emailed
// its way through the prospect list before anyone opened the Backlinks page.
// 25 is roughly treble the busiest real day so far; the sweep goes quiet for
// the rest of the day when it is reached and says so in its result.
...
//
// The hunting itself lives in lib/contact-hunt.js: it follows the site's own
// contact/press navigation and decodes the obfuscations (Cloudflare cfemail,
// bracketed [at]/[dot]) that hid addresses from the old fixed-path pass.
export async function resolveContact(site, website) {
  ...
  let candidate = await huntContact(domain);
...
export function renderOutreach(site, row, brand, { followUp = false, outreach = null } = {}) {
  ...
  // The title's own brand colour from its site theme, not accentHex, which was
  // set by hand at launch and had drifted (Fleet amber, Barbering gold).
  const accent = paletteFor(site?.slug)?.brand || site?.accentHex || "#2642e0";
...
export async function runOutreachSends(site, creds) {
  ...
  // The day's budget, counted from what actually left: first sends stamp
  // sentAt, chases stamp followUpSentAt, and both count. UTC midnight is the
  // boundary because every other clock in the engine is UTC.
  const dayStart = new Date();
  dayStart.setUTCHours(0, 0, 0, 0);
  const sentToday =
    (await db.outreachEmail.count({ where: { sentAt: { gte: dayStart } } })) +
    (await db.outreachEmail.count({ where: { followUpSentAt: { gte: dayStart } } }));
  let budget = MAX_SENDS_PER_DAY - sentToday;
  if (budget <= 0) {
    return { skipped: `daily send cap reached (${sentToday}/${MAX_SENDS_PER_DAY}); resumes at midnight UTC` };
  }

  const approved = await db.outreachEmail.findMany({
    // Approved first, oldest first: a human fast-track should never queue
    // behind the agent's drafts when a sweep hits the per-run cap. The daily
    // budget is spent in the same order - first sends before chases - so the
    // cap can never starve a fast-track in favour of a follow-up.
    where: { status: { in: ["approved", "pending"] }, contactEmail: { not: null } },
    orderBy: [{ status: "asc" }, { createdAt: "asc" }],
    take: Math.min(MAX_SENDS_PER_RUN, budget),
  });
  ...
  const chases = budget <= 0 ? [] : await db.outreachEmail.findMany({
    where: {
      status: "sent",
