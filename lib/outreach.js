...
function signOff(brand, outreach) {
...
export function renderOutreach(row, brand, { followUp = false, outreach = null } = {}) {
  const s = signOff(brand, outreach);
  const logo = logoUrl();
...
export async function sendOutreachEmail(site, id, { followUp = false, creds = null } = {}) {
  ...
  if (!isSendConfigured(creds)) {
    return refuse(row, gmailSetupHint(creds?.outreach) || "Sending is not configured.", followUp);
  }

  const { subject, text, html } = renderOutreach(row, row.brand, { followUp, outreach: creds.outreach });
  try {
    await sendGmail({
