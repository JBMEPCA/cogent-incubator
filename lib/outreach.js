...
const MAX_LINK_CHECKS_PER_RUN = 12;
...
const SITE_HOST = "smartsme.co.uk";

export function isOutreachConfigured() {
  return isDraftingConfigured() && isWordPressConfigured();
}
...
export function outreachSetupHint() {
  if (!isDraftingConfigured()) return "Needs ANTHROPIC_API_KEY to compose outreach.";
  if (!isWordPressConfigured()) return "Needs WordPress credentials to resolve article URLs.";
  return gmailSetupHint();
}
...
const JUNK_DOMAINS = ["example.com", "sentry.io", "wixpress.com", "godaddy.com", "squarespace.com"];
...
async function fetchText(url, timeoutMs = 8000) {
  ...
  try {
    const res = await fetch(url, {
      ...
      redirect: "follow",
      headers: { "user-agent": "SmartSMEBot/1.0 (+https://smartsme.co.uk)" },
    });
...
function harvestEmails(html, domain) {
  ...
  const list = [...found];
  const onDomain = domain ? list.filter((e) => e.endsWith(`@${domain}`) || e.includes(domain)) : list;
...
export async function resolveContact(website) {
  ...
  for (const path of CONTACT_PATHS) {
    ...
    if (email) return { email, confidence: "found" };
  }
  return { email: `press@${domain}`, confidence: "guessed" };
}

// ---------- composing ----------

const COMPOSE_SYSTEM = `You are James Burke, editor of Smart SME (smartsme.co.uk), a UK publication covering how small businesses adopt AI, software and automation. You are writing the email yourself, in the first person, and it is signed off in your name. Never refer to James Burke or to "the editor" in the third person: that is you.

Smart SME has published an article that mentions one or more companies. Your job is to identify those companies and write, for each, an email telling them they were featured.
...
- Only list companies genuinely written about. Smart SME itself, and any publication or platform named only as a source, are not mentions.
...
async function composeMentions(input) {
  ...
  try {
    (await import("./agents/meter")).recordUsage(response.model || "claude-opus-4-8", response.usage);
  } catch {}
...
const markScanned = (articleId) =>
  prisma.article.update({ where: { id: articleId }, data: { outreachScannedAt: new Date() } });

export async function runBacklinkOutreach() {
  if (!isOutreachConfigured()) throw new Error("outreach engine not configured");
  ...
  const brands = await prisma.prBrand.findMany();
  ...
  for (const article of articles) {
    ...
    try {
      const post = await fetchPost(article.wpPostId);
      url = post.link;
    ...
    let mentions = [];
    try {
      mentions = await composeMentions({ title, url, text, knownBrands });
    } catch (e) {
    ...
    for (const m of mentions) {
      ...
      if (!brand) {
        brand = await prisma.prBrand.create({
          data: {
      ...
      if (!brand.prContactEmail && (brand.website || m.website)) {
        const contact = await resolveContact(brand.website || m.website);
        if (contact) {
          brand = await prisma.prBrand.update({
            where: { id: brand.id },
            data: {
      ...
      await prisma.outreachEmail.create({
        data: {
          ...
          subject: stripEmDashes(String(m.subject || `Smart SME has covered ${brand.name}`)).slice(0, 160),
...
export const ANCHOR_TEXT = "As seen on Smart SME";
...
function signOff(brand) {
  ...
  // untidy but non-compliant, so a missing env var must not be able to produce
  // one. Override with OUTREACH_POSTAL_ADDRESS to give the full registered address.
  const address = process.env.OUTREACH_POSTAL_ADDRESS || "Cogent Multimedia Ltd, United Kingdom";
  const unsub = unsubscribeUrl(brand?.id);
  return {
    name,
    address,
    unsub,
    reason: `You are getting this because Smart SME published an article about ${brand?.name || "your company"}.`,
  };
}

export function renderOutreach(row, brand, { followUp = false } = {}) {
  const s = signOff(brand);
  const logo = logoUrl();
  ...
  const html = `<div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;font-size:15px;line-height:1.55;color:#111;max-width:560px">
    ...
    <p style="margin:0 0 16px">${escapeHtml(s.name)}<br><span style="color:#6b7280">Editor, Smart SME</span></p>
    <hr style="border:none;border-top:1px solid #e2e5ee;margin:0 0 12px">
    ${
      logo
        ? `<p style="margin:0 0 8px"><a href="https://${SITE_HOST}"><img src="${logo}" alt="Smart SME" height="22" style="height:22px"></a></p>`
        : ""
    }
...
async function refuse(row, message, followUp) {
...
export async function sendOutreachEmail(id, { followUp = false } = {}) {
  const row = await prisma.outreachEmail.findUnique({ where: { id }, include: { brand: true } });
  if (!row) throw new Error("outreach email not found");
  if (!row.contactEmail) return refuse(row, "No contact address on this row.", followUp);
  if (row.brand?.optedOut) return refuse(row, `${row.brandName} has opted out.`, followUp);
  ...
  if (isUnreachable(row.brandName)) {
    return refuse(row, `${row.brandName} is too large to answer press outreach. Not sent.`, followUp);
  }
  ...
  if (!isSendConfigured()) return refuse(row, gmailSetupHint() || "Sending is not configured.", followUp);

  const { subject, text, html } = renderOutreach(row, row.brand, { followUp });
  try {
    await sendGmail({
      to: row.contactEmail,
...
export async function runOutreachSends() {
  if (!isSendConfigured()) return { skipped: gmailSetupHint() };

  const approved = await prisma.outreachEmail.findMany({
  ...
  for (const row of approved) {
    try {
      await sendOutreachEmail(row.id);
      sent += 1;
    } catch (e) {
  ...
  const chases = await prisma.outreachEmail.findMany({
    where: {
      status: "sent",
  ...
  for (const row of chases) {
    try {
      await sendOutreachEmail(row.id, { followUp: true });
      chased += 1;
    } catch (e) {
      failures.push({ id: row.id, brand: row.brandName, error: e.message });
    }
...
export async function runBacklinkCheck() {
  ...
  for (const row of rows) {
    const target = row.brand?.newsHubUrl || row.brand?.website;
    if (!target) continue;
    const html = await fetchText(target);
    if (!html) continue;
    if (!html.includes(SITE_HOST)) continue;
    await prisma.outreachEmail.update({
      where: { id: row.id },
      data: { status: "linked", linkedAt: new Date(), linkUrl: target },
    });
    linked += 1;
  }
...
export async function runBounceCheck() {
  const rows = await prisma.outreachEmail.findMany({
  ...
  const { available, addresses } = await bouncedSince(oldest);
  ...
  for (const row of rows) {
    ...
    if (row.brandId) {
      await prisma.prBrand.updateMany({
...
export async function runReplyCheck() {
  ...
  const replied = [];
  for (const row of rows) {
    const hit = await repliedSince(row.contactEmail, row.sentAt);
    if (hit === null) return { checked: 0, replied, available: false };
    if (!hit) continue;
    await prisma.outreachEmail.update({ where: { id: row.id }, data: { status: "replied" } });
...
export async function dueToRun(key, minHours) {
  const setting = await prisma.engineSetting.findUnique({ where: { key } });
...
export async function outreachStats() {
  const rows = await prisma.outreachEmail.groupBy({ by: ["status"], _count: { _all: true } });
