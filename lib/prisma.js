...
const TENANTED = new Set([
  ...
  "AudienceSnapshot",
  "TargetAchievement",
  // Interview targets were missing from this list, so forSite() passed them
  // through unscoped. Every title's hourly sweep then read all five titles'
  // queues, and whichever sweep reached a due row first sent it under its own
  // masthead: on 14 Sep 2026 Royal Aberdeen's course manager was chased by
  // Airport Business Magazine, and nine people in all were written to by the
  // wrong title. The same leak reached the drafting step, which writes to a
  // title's WordPress.
  "InterviewTarget",
]);
