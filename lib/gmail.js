...
export function outreachSender() {
  ...
  return {
    email,
    name: process.env.OUTREACH_FROM_NAME || "James Burke",
    replyTo: process.env.OUTREACH_REPLY_TO || email,
  };
}

export function isGmailConfigured() {
  return Boolean(isGoogleConfigured() && process.env.OUTREACH_FROM_EMAIL);
}
...
export function gmailSetupHint() {
  ...
  if (!process.env.OUTREACH_FROM_EMAIL)
    return "Set OUTREACH_FROM_EMAIL to the mailbox outreach should be sent from.";
  return null;
}
...
function buildMime({ from, to, subject, text, html, replyTo, unsubscribeUrl, unsubscribeMailto }) {
  const boundary = `smartsme-${b64url(`${subject}${to}`).slice(0, 24)}`;
  const headers = [
    `From: ${from}`,
    `To: ${to}`,
...
export async function sendGmail({
  ...
  if (!sender) throw new Error(gmailSetupHint() || "Gmail sending is not configured.");
...
export async function repliedSince(fromEmail, sinceDate) {
  if (!isGmailConfigured() || !fromEmail) return null;
  const sender = outreachSender();
...
export async function bouncedSince(sinceDate, max = 40) {
  const empty = { available: false, addresses: new Map() };
  if (!isGmailConfigured()) return empty;
  const sender = outreachSender();
  const after = new Date((sinceDate ? new Date(sinceDate).getTime() : Date.now()) - 86400000)
    .toISOString()
    .slice(0, 10)
