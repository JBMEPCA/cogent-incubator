...
function buildMime({ from, to, subject, text, html, replyTo, unsubscribeUrl, unsubscribeMailto, inReplyTo }) {
  ...
  const headers = [
    ...
    // Answering a live conversation out of thread reads like a different
    // sender and loses everything the reader already agreed to. Both headers,
    // because clients disagree about which one they honour.
...
export async function sendGmail({
  ...
  unsubscribeUrl,
  unsubscribeMailto,
  // Message-Id of the mail being answered, and the Gmail thread it sits in.
  // Supply both: the headers make other clients thread it, threadId makes
  // Gmail file our own copy in the same conversation.
  inReplyTo,
  threadId,
}) {
...
/**
 * Inbound mail matching a Gmail query, with bodies.
 *
 * repliedSince() answers "did THIS address write back", which is the wrong
 * question for the interview franchise and quietly lost us a yes. Of the first
 * three replies, one came from a different mailbox at the same company after a
 * shared inbox forwarded it internally, and one came from the subject's PR
 * agency on a different domain entirely. Both are invisible to a from: check
 * and both were real.
 *
 * So: sweep by subject, read the body, and let the caller work out who it
 * belongs to.
 */
export async function inboundMatching(outreach, query, max = 25) {
  if (!isGmailConfigured(outreach)) return [];
  const sender = outreachSender(outreach);
  ...
  for (const m of messages) {
    const res = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${m.id}?format=full`, auth);
    if (!res.ok) continue;
    const md = await res.json();
    const h = Object.fromEntries((md.payload?.headers || []).map((x) => [x.name.toLowerCase(), x.value]));
    out.push({
      id: md.id,
      threadId: md.threadId,
      from: h.from || "",
      fromEmail: (h.from || "").replace(/.*<|>.*/g, "").trim().toLowerCase(),
      subject: h.subject || "",
      messageId: h["message-id"] || "",
      date: h.date ? new Date(h.date) : null,
      body: textOf(md.payload),
    });
  }
  return out;
}
...
/**
 * The image attachments on one message, largest first.
 *
 * The interview franchise asks every subject for "a high-res headshot you are
 * happy for us to use" and promises "we publish a full profile with your
 * photo". They send one. Nothing read it, so the promise was broken on every
 * instalment and the headshotUrl column stayed null.
 *
 * Signature furniture is the whole difficulty. A reply from anyone with a
 * corporate footer carries two or three inline images that are logos and
 * address bars, and they arrive as attachments exactly like the portrait does.
 * Two cheap filters separate them: Outlook names inline signature parts
 * imageNNN.ext, and a usable headshot is far larger than a logo. Brenda
 * Sunley's reply carried a 42KB imageNNN banner, a 162KB imageNNN square and
 * the 8.9MB portrait we actually wanted.
 */
export async function imageAttachments(outreach, messageId, { minBytes = 200000, maxBytes = 12000000 } = {}) {
  if (!isGmailConfigured(outreach)) return [];
  const sender = outreachSender(outreach);
  if (!sender) return [];
  ...
  (function walk(p) {
    if (!p) return;
    if (p.filename && p.mimeType && p.mimeType.startsWith("image/")) parts.push(p);
    for (const child of p.parts || []) walk(child);
  })(msg.payload);

  const wanted = parts
    .filter((p) => p.body?.attachmentId)
    .filter((p) => (p.body.size || 0) >= minBytes)
    // The sweep runs inside a 60 second function, so an enormous original is
    // skipped rather than allowed to time the whole tick out. Anything this
    // big is a print file and wants handling by a person anyway.
    .filter((p) => (p.body.size || 0) <= maxBytes)
    // Inline signature images, which are never the headshot.
    .filter((p) => !/^image[0-9]+[.]/i.test(p.filename))
  ...
  for (const p of wanted) {
    ...
    out.push({
      ...
      data: Buffer.from(String(a.data).replace(/-/g, "+").replace(/_/g, "/"), "base64"),
    });
  }
  return out;
}
