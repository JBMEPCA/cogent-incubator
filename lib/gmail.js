...
export async function listInbox(outreach, { max = 25 } = {}) {
  ...
  const messages = (
    await Promise.all(
      ids.map(async ({ id }) => {
        const res = await fetch(
          `https://gmail.googleapis.com/gmail/v1/users/me/messages/${id}?format=metadata&metadataHeaders=From&metadataHeaders=Subject&metadataHeaders=Date&metadataHeaders=List-Unsubscribe&metadataHeaders=Auto-Submitted&metadataHeaders=Precedence`,
          { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" }
        );
        if (!res.ok) return null;
        ...
        return {
          ...
          // dashboard. Real correspondence carries none of the three.
          listUnsubscribe: Boolean(h["list-unsubscribe"]),
          autoSubmitted: (h["auto-submitted"] || "no").toLowerCase() !== "no",
