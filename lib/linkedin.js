...
import { forSite } from "./prisma.js";
import { ukHour } from "./agents/hours.js";
...
export async function mentionsForPost(site, post, auth) {
  ...
  // The anchor text comes with the URL: it is the brand as the writer typed
  // it, and it is what lets a run-together domain resolve to a hyphenated
  // LinkedIn vanity name. Bare hrefs (an image wrapped in a link, say) still
  // count as links, just without an anchor to learn from.
  const links = [];
  for (const m of article.body.matchAll(/<a[^>]+href=["']([^"']+)["'][^>]*>(.*?)<\/a>/gis)) {
    links.push({ url: m[1], text: m[2].replace(/<[^>]+>/g, "").trim() || null });
  }
  for (const m of article.body.matchAll(/href=["']([^"']+)["']/gi)) {
    if (!links.some((l) => l.url === m[1])) links.push({ url: m[1], text: null });
  }
