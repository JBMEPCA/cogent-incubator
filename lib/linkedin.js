...
async function readAuth() {
  const row = await prisma.engineSetting.findUnique({ where: { key: AUTH_KEY } });
  if (!row) return null;
  try {
...
export async function disconnect() {
  await prisma.engineSetting.deleteMany({ where: { key: AUTH_KEY } });
}
...
export async function imageForPost(post) {
...
export async function publishPost(post) {
  const auth = await usableToken();

  const { url, alt } = await imageForPost(post);
  ...
  const body = {
    ...
    content: {
      media: { id: imageUrn, altText: (alt || "Smart SME Magazine").slice(0, 300) },
    },
  };
...
export async function nextPostingSlot() {
  const last = await prisma.linkedInPost.findFirst({
