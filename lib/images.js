...
// Each gate attempt is one vision call. This was five while the gate argued
// about relevance and needed a deep shortlist to find one it would accept.
// From 1 Sep 2026 the gate defaults to yes and refuses only watermarks, unsafe
// pictures, unusable files and a wrong company's livery, so the first or second
// candidate is taken almost every time and the rest were pure spend.
...
//
// ANY source, on JB's explicit instruction of 30 August 2026: "when I say don't
// care about image rights, I mean it... I'm telling you explicitly you can use
// any images with no rights concerns." He was asked, he answered twice, and the
// second time was because a restriction I had added on my own judgement cost
// Smart SME and Golf a whole Sunday's publishing.
//
// ONE hard rule survives, and it is his: never a watermarked image. That is
// enforced at the gate, which can see the pixels, and by the filename filter
// below for the cases that announce themselves.
//
// Kept as a list because the host still decides the credit line, and because
// knowing an image came from a competing title is worth recording even when it
// is allowed.
...
// Watermark-bearing library previews usually say so in the path, and a stock
// preview is the one thing JB has ruled out outright.
const NOT_A_PHOTO =
  /(logo|icon|favicon|sprite|avatar|badge|button|spacer|pixel|tracking|placeholder|1x1|blank|watermark|preview|comp[-_]?image|shutterstock|gettyimages|alamy|dreamstime|istockphoto|123rf|depositphotos)/i;
 ...
 */
export async function sourceImage(site, sourceUrl, { allowPublishers = true } = {}) {
  ...
  for (const raw of candidates) {
    ...
    // Prove it is a real, header-sized image in a format vision can actually
    // read before offering it to the gate. SVG and AVIF need sharp to become
    // viewable, and sharp is not reliably present on the deployment - it went
    // missing on 20 August and again on 28 - so handing one over produces
    // "picture gate cannot process this image" and burns an attempt on a
    // tooling fault dressed as an editorial one.
    let ok = false;
    try {
      const res = await fetch(url, { headers: UA, signal: AbortSignal.timeout(10000) });
      ...
      ok =
        ...
        (await res.arrayBuffer()).byteLength > 15000;
    } catch {
      ok = false;
    }
    if (!ok) continue;
