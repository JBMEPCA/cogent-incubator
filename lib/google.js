...
function loadServiceAccount() {
  ...
  if (inline) {
    // Strip a BOM, surrounding quotes, and an accidentally-pasted KEY= prefix
    // before deciding what this is. Env UIs add all three, and each one turns a
    // valid key into "not JSON" further down: a value that no longer starts with
    // "{" gets base64-decoded instead, which yields binary and a parser error
    // naming a character nobody can search for.
    const raw = inline
      .trim()
      .replace(/^﻿/, "")
  ...
  // A raw JSON.parse here surfaces on the Analytics page as
  // `Unexpected token '�', "� o�'..." is not valid JSON`, which says nothing about
  // what is actually wrong or where to fix it. The fault is always the stored
  // value, never the key file, so name the variable and the remedy.
  let sa;
  try {
    sa = JSON.parse(text);
  } catch {
    const source = inline ? "GOOGLE_SERVICE_ACCOUNT_JSON" : `the file at ${path}`;
    throw new Error(
      `${source} is set (${(inline || text || "").length} chars) but did not decode to JSON. ` +
        `On Vercel it must be the base64 of the service-account key file — no surrounding ` +
        `quotes, no "KEY=" prefix, and not truncated. Re-push it from .env.vercel.`
    );
  }
