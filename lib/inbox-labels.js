...
export async function runInboxLabels({ days = 2, maxMessages = 150, maxThreads = 150 } = {}) {
  ...
  const creds = await db.siteCredential.findMany({
    where: { kind: "outreach" },
  ...
  for (const c of creds) {
    ...
    let payload;
    try {
      payload = decryptJson(c.payloadEnc);
    } catch {
      continue;
    }
    if (payload?.fromEmail) titleByAddress.set(payload.fromEmail.toLowerCase(), label);
  }
