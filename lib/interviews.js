...
export async function draftInterviewArticle(site, target, { db, anthropic, wp, franchise, siteUrl, houseStyle, publish, makeArticle }) {
  ...
  const title = stripEmDashes(`<span class="franchise-eyebrow">${franchise}${/ with$/i.test(franchise) ? "" : ":"}</span> ${plan.headline}`);
  const plainTitle = stripEmDashes(`${franchise}${/ with$/i.test(franchise) ? "" : ":"} ${plan.headline}`);
...
export async function runInterviewSweep(
  ...
  // siteId on every query in this sweep, spelled out. InterviewTarget is not in
  // TENANTED (lib/prisma.js), because the fleet /interviews page reads it across
  // titles, so forSite() passes these through unscoped. Without it each title's
