...
/**
 * Pick the address to write to, checking each candidate against a live SMTP
 * probe before it is used.
 *
 * This replaced a guess-then-bounce cascade, which does not work: most small
 * business domains are catch-all, so an invented address is accepted, no bounce
 * ever arrives, and the mail lands in an unread pile or in front of the wrong
 * person. Bouncing tells you an address is dead; NOT bouncing tells you
 * nothing at all, so the old design could report eleven successes having
 * reached nobody.
 *
 * Verifying first inverts that. MillionVerifier answers per mailbox, so
 * "ok" means the mailbox exists and a personal address can be used with
 * confidence, while "catch_all" means the domain accepts everything and the
 * answer is genuinely unknowable. Measured over the first eleven founders:
 * nine had a reachable personal mailbox, two were catch-all.
 *
 * Order of preference:
 *   1. a personal address the probe confirms exists
 *   2. an address a human published (press inbox, then site, then generic)
 *   3. nothing. A catch-all guess is never sent to.
 */
export async function resolveAddress(target, { verify } = {}) {
...
export async function sendPreAsk(db, target, { outreach, titleName, franchise, siteUrl, senderName, verify }) {
  const pick = await resolveAddress(target, { verify });
  ...
  const body = buildPreAsk({
    ...
    siteUrl,
    senderName,
    viaGeneric: pick.source === "generic" || isGenericAddress(pick.email),
  });
 ...
 */
export async function sendFollowUp(db, target, { outreach, titleName, siteUrl, senderName, verify }) {
  ...
  const fresh = isGenericAddress(target.email) ? await resolveAddress(target, { verify }) : null;
