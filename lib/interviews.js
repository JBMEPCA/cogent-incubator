...
import { sendGmail, isGmailConfigured, inboundMatching, imageAttachments } from "./gmail";
import { uploadMedia, findMediaByUrl, resolveCategory } from "./wordpress";
...
export async function resolveAddress(target, { verify, hunt } = {}) {
  ...
  // A no-reply box is never fresh, whichever branch offers it. Putting the
  // test here rather than on the hunt branch alone covers seed data too: a
  // human copying an address off a contact page can paste one just as easily.
  const fresh = (a) => a && !tried.has(a.toLowerCase()) && !NO_REPLY.test(a);
...
/**
 * Addresses that are published and useless.
 *
 * The hunter reads whatever the page prints, and plenty of sites print a
 * no-reply box in a footer or a schema block. Sending an interview request
 * there is not a long shot, it is a guaranteed nothing, and it burns the row:
 * the send is recorded, the nudge follows three days later, and the person we
 * wanted to reach never sees either. germinalamenity.com is the one that
 * caught this.
 */
export const NO_REPLY =
  /^(no-?reply|do-?not-?reply|donotreply|bounce|mailer-daemon|postmaster|unsubscribe)@/i;
 ...
 */
export function buildFollowUp({ personName, newsHook, questions, titleName, titleDescriptor, siteUrl, senderName, viaGeneric, firstContact, since = "last week" }) {
  const first = greetingName(personName);
  const qs = (Array.isArray(questions) ? questions : String(questions || "").split("\n"))
    .map((q) => String(q).replace(/^\s*\d+[.)]\s*/, "").trim())
    .filter(Boolean);
  ...
  const opening = firstContact
    ...
    // `since` because this mail is also used to rescue the August pre-asks,
    // where "last week" would be a month out and the first thing they read.
    : `I wrote to you ${since} about featuring you in ${titleName}. I suspect it read like an advert, so let me be plain about it: this is free, there is nothing to buy, and there is no advertising attached to it.`;
...
/**
 * The nudge, seven to eight days after the questions went out.
 *
 * NOT buildFollowUp. That one opens "I wrote to you last week about featuring
 * you in X, I suspect it read like an advert", which was written to apologise
 * for the pre-ask and makes no sense to someone who already has the questions
 * sitting in their inbox. Sending it to them would read as though we had lost
 * track of our own correspondence.
 *
 * Short on purpose. The ask has already been made in full; this only has to
 * clear the two things that actually stop a reply, which are forgetting and
 * not wanting to say no.
 */
export function buildNudge({ personName, company, titleName, senderName, siteUrl }) {
  const first = greetingName(personName);
  return stripEmDashes(
    [
      `Hi ${first},`,
      "",
      `I sent some questions over last week about featuring ${company} in ${titleName}, so this is just in case it went astray.`,
      "",
      "There is no deadline and no need to answer all seven. One good answer is worth more to me than seven short ones.",
      "",
      "And if it is simply not for you, say so and I will leave you in peace. No hard feelings either way.",
      "",
      "Best,",
      senderName,
      `Publisher, ${titleName}`,
...
/**
 * The chase for someone who said yes and then went quiet.
 *
 * buildNudge is written for silence and opens "in case it went astray", which
 * is wrong for a person who already replied to say they were in: it reads as
 * though we had lost track of our own correspondence. These people are the
 * warmest leads in the queue, so they get the questions again and an easy way
 * out. Added 23 Sep 2026, when seven of them turned out to be sitting in the
 * queue getting no chase at all because the nudge excluded agreedAt.
 */
export function buildAgreedChase({ personName, company, questions, titleName, senderName, siteUrl }) {
  const first = greetingName(personName);
  const qs = (Array.isArray(questions) ? questions : String(questions || "").split("\n"))
    .map((q) => String(q).replace(/^\s*\d+[.)]\s*/, "").trim())
    .filter(Boolean);
  ...
  return stripEmDashes(
    [
      `Hi ${first},`,
      "",
      `You kindly said yes to an interview about ${company} for ${titleName}, and I have not heard since, which is usually a sign I have not made it easy enough. So here are the questions again, all in one place.`,
      "",
      ...qs.map((q, i) => `${i + 1}. ${q}`),
      "",
      "There is no deadline and no need to answer all of them. One good answer is worth more to me than seven short ones, and a photo of you is all we need alongside it.",
      "",
      "If it is no longer for you, just say so and I will leave you in peace. No hard feelings either way.",
      "",
      "Best,",
      senderName,
      `Publisher, ${titleName}`,
...
export async function interviewStats(db, siteId) {
  ...
  /*
   * "Said yes" is counted from agreedAt, NOT from status.
   *
   * It used to be `agreed + questioned + answered + drafted + published`, which
   * was right for exactly as long as the flow was two steps: under the pre-ask,
   * "questioned" could only be reached by someone who had already said yes.
   *
   * sendQuestionsUpFront() broke that on 2 Sep 2026. A new batch now lands on
   * "questioned" from the very first cold email, because they have the
   * questions, and nobody has agreed to anything. The dashboard duly reported
   * fourteen yeses across Fleet and Golf from people who had not replied at
   * all, and a reply rate to match.
   *
   * agreedAt is set only when a human actually engaged: by the sweep when it
   * classifies a reply as a yes or as answers, and never by an outbound send.
   * So it is the honest signal, and it needs no schema change.
   */
  const agreed = await db.interviewTarget.count({
    where: { ...(where || {}), OR: [{ agreedAt: { not: null } }, { status: { in: ["answered", "drafted", "published"] } }] },
  });
...
/**
 * Every quote in the piece has to be something the subject actually typed.
 *
 * This is the guard that matters most in the whole file. The rest of the
 * pipeline can be wrong and cost us a reply; a fabricated quote attributed to a
 * named real person under the masthead is a different category of mistake.
 * Whitespace and smart quotes are normalised because a model will tidy those
 * without being asked, and that is not the same as inventing words.
 */
const normalise = (s) =>
  String(s || "")
    .replace(/[‘’]/g, "'")
...
export function quoteIsVerbatim(quote, source) {
...
/**
 * Turn seven answers into the locked article format, as a WordPress DRAFT.
 *
 * Deliberately stops at draft. Publishing an unread, machine-written article
 * about a named real person is the one step in this chain that cannot be taken
 * back, so a human keeps it. Everything up to that point is now automatic.
 */
export async function draftInterviewArticle(site, target, { db, anthropic, wp, franchise, siteUrl, houseStyle, publish, makeArticle }) {
  if (!target.replyBody) return { drafted: false, reason: "no answers" };
  if (target.articleId) return { drafted: false, reason: "already drafted" };

  const asked = String(target.questions || "").split("\n").filter(Boolean);
  const res = await anthropic.messages.create({
    model: "claude-sonnet-5",
    max_tokens: 4000,
    system:
      `${houseStyle || ""}\n\n` +
      "You are writing one instalment of a recurring interview franchise. The subject answered questions by email and those answers are the only original material on the page, so they carry the piece and your prose is joinery between them.\n\n" +
      "Rules that are not negotiable:\n" +
      "- Every string in `quote` MUST be copied character for character from the subject's answers. Never tidy, never merge two answers, never write a quote they did not type. If an answer is not quotable, leave that section without a quote.\n" +
      "- Never state a fact that is not in the answers or the brief. No founding years, no turnover, no employee counts, no awards you were not told about.\n" +
      "- No em dashes or en dashes anywhere.\n" +
      "- UK spelling.\n" +
      "- The headline names the person and says what the piece is about. Do not include the franchise name; it is added separately.\n\n" +
      "Reply with JSON only:\n" +
      '{"headline":"","standfirst":"","metaDesc":"","keyphrase":"","sector":"two or three words",' +
      '"based":"the town and county the company operates from, ONLY if the answers or their signature say so, otherwise an empty string",' +
      '"companyBlurb":"40 to 60 words on what the company does and who buys from it",' +
      '"sections":[{"heading":"","paragraphs":["",""],"quote":""}]}\n' +
      "Four to six sections. The first paragraph of the first section opens the piece.",
    messages: [
      {
        role: "user",
        content:
          `Franchise: ${franchise}\nSubject: ${target.personName}, ${target.personRole || ""}\n` +
          `Company: ${target.company}\nWhy we approached them: ${target.newsHook}\n\n` +
          `Questions asked:\n${asked.map((q, i) => `${i + 1}. ${q}`).join("\n")}\n\n` +
          `Their answers, verbatim:\n${String(target.replyBody).slice(0, 30000)}`,
      },
    ],
  });
  ...
  try {
    ...
    return { drafted: false, reason: "model did not return usable JSON" };
  }

  // Drop any quote that is not actually theirs, and say how many went.
  let dropped = 0;
  const sections = (plan.sections || []).map((s) => {
    if (s.quote && !quoteIsVerbatim(s.quote, target.replyBody)) { dropped++; return { ...s, quote: null }; }
    return s;
  });
  ...
  const body = stripEmDashes(
    [
      ...
      `<div class="interview-company-head"><h2>Who are ${esc(target.company)}?</h2></div>`,
      `<p>${esc(plan.companyBlurb)}</p>`,
      `<dl>` +
        [
          ["Company", esc(target.company)],
          ["Website", target.companyDomain ? `<a href="https://${target.companyDomain}/">${esc(target.companyDomain)}</a>` : ""],
          ["Sector", esc(plan.sector)],
          ["Based", esc(plan.based)],
        ]
          .filter(([, v]) => v)
        ...
        `</dl>`,
      `</section>`,
      `<p><strong>${esc(plan.standfirst)}</strong></p>`,
      ...sections.flatMap((s, i) => [
        i === 0 ? "" : `<h2 class="wp-block-heading">${esc(s.heading)}</h2>`,
        ...(s.paragraphs || []).map((p) => `<p>${esc(p)}</p>`),
        s.quote ? `<blockquote class="wp-block-quote interview-quote"><p>${esc(s.quote)}</p></blockquote>` : "",
      ]),
      `<p><em>${esc(franchise)} profiles the people behind the businesses we cover. Answers have been lightly edited for punctuation only.</em></p>`,
    ]
      .filter(Boolean)
      .join("\n\n")
  );

  // An optional per-title glyph on the franchise eyebrow.
  //
  // Deliberately NOT part of the franchise name, because that same string
  // opens every approach email this franchise sends. An emoji in a cold
  // subject line reads as marketing rather than as an editor writing to you,
  // and the franchise depends on the opposite impression.
  const emojiRow = await db.engineSetting.findUnique({ where: { key: EMOJI_KEY } });
  const eyebrow = [String(emojiRow?.value || "").trim(), franchise].filter(Boolean).join(" ");

  const title = stripEmDashes(`<span class="franchise-eyebrow">${eyebrow}${/ with$/i.test(franchise) ? "" : ":"}</span> ${plan.headline}`);
  const plainTitle = stripEmDashes(`${eyebrow}${/ with$/i.test(franchise) ? "" : ":"} ${plan.headline}`);

  // The permalink, built from the words rather than the decorated title.
  //
  // WordPress derives the slug from post_title, and post_title carries the
  // eyebrow markup and, on titles that set one, an emoji. The first piece
  // published this way landed on a percent-encoded truck at the front of
  // its URL, which is unshareable and looks broken in a search result.
  // Deriving it here lets the title stay as decorated as the franchise likes.
  ...
  // The photograph they were asked for.
  //
  // The pre-ask promises "we publish a full profile with your photo" and asks
  // for a high-res headshot. Subjects send one, the sweep now files it, and
  // until this existed nothing read it back: every instalment published with
  // no picture of the person it is about, against a promise made in writing.
  let featuredMediaId = null;
  if (target.headshotUrl) {
    try {
      featuredMediaId = await findMediaByUrl(wp, target.headshotUrl);
    } catch {
      featuredMediaId = null;
    }
  }

  // Without this the post lands in Uncategorised. The category was written to
  // the Article row and never to WordPress, so the app and the site disagreed
  // about where every interview lived. That is the same taxonomy split that put
  // a whole batch of Fleet's launch articles in Uncategorised.
  let categoryId = null;
  try {
    categoryId = await resolveCategory(wp, "Case Studies");
  } catch {
    categoryId = null;
  }

  const post = await publish(wp, {
    title,
    slug,
    body,
    status: "draft",
  ...
  await db.interviewTarget.update({
    ...
    data: {
      ...
      // No publishedUrl and no publishedAt: it is not published, and the
      // backlink ask keys off publishedAt so setting it here would email the
      // subject a link to something nobody can read.
      error: dropped ? `${dropped} quote(s) dropped as not verbatim` : null,
    },
  });

  return { drafted: true, wpPostId: post.id, quotesDropped: dropped, title: plainTitle };
}
...
const EMOJI_KEY = "interview_franchise_emoji";
 ...
 */
export async function runInterviewSweep(
  site,
  { db, creds, anthropic, siteUrl, draft, now = new Date(), maxSends = 5, foreignAddresses = [] }
) {
  ...
  const out = { franchise, agreed: 0, answered: 0, declined: 0, unclear: 0, questionsSent: 0, drafted: 0, nudges: 0, backlinkAsks: 0, notes: [] };

  // siteId on every query in this sweep, spelled out, even though forSite() now
  // scopes InterviewTarget too (lib/prisma.js). It did not until 14 Sep 2026, and a
  // second guard costs nothing here. Without either, each title's
  // sweep worked the whole fleet's queue under its own name: on 14 Sep 2026
  // Airport Business Magazine chased two Smart SME founders and a golf club, and
  // Smart SME chased a Barbering Business interviewee.
  const mine = { siteId: site.id };

  // ---- 1. what came back ----
  const open = await db.interviewTarget.findMany({ where: { ...mine, status: { in: ["asked", "questioned"] } } });
  if (open.length) {
    ...
    // Since 8 Sep 2026 the other four titles forward a copy of everything into
    // jb@smartsme.co.uk, which is not some third mailbox but Smart SME's own.
    // So SME's sweep now sees Golf's and Barbering's interview replies sitting
    // in its inbox, reads them by subject like any other, and reports them as
    // unmatched. Harmless while no two titles chase the same company, and a
    // Golf interview drafted into Smart SME the day they do.
    //
    // Both directions, and the second one is not obvious. A reply is addressed
    // to the title that wrote to it, so `-to:` covers the inbound copies. But
    // `-from:me` only means this mailbox's own address, so the four other
    // titles' SENT asks — every one of them now sitting in here after the
    // history import — sail straight through it. Hence `-from:` as well.
    const notForUs = foreignAddresses
      .filter((a) => a && a.toLowerCase() !== outreach?.fromEmail?.toLowerCase())
      .map((a) => `-to:${a} -from:${a}`)
      .join(" ");

    const mail = await inboundMatching(
      outreach,
      `-from:me newer_than:21d (subject:"Seven questions for" OR subject:"Featuring you in") ${notForUs}`.trim(),
      25
    );
    ...
    for (const msg of mail) {
      ...
      // Stored whole. This was cut at 8,000 characters while the drafter reads
      // up to 30,000, and Penny Joyner-Platt's answers ran to about 9,700, so
      // her draft was written without the end of her last answer. Worse, the
      // quote check verifies against this field, so a genuine quote from the
      // cut-off part is discarded as unverifiable. The cap is only a backstop
      // against a pasted document, not a budget.
      const data = { replyBody: stripQuotedReply(msg.body).slice(0, 60000) };
      if (verdict.verdict === "declined") {
        ...
        out.answered++;
        // File the headshot they were asked for.
        //
        // This is the only point in the pipeline that still knows which
        // message the answers arrived in, and the attachment is reachable
        // only from that message, so it cannot wait until drafting. Failure
        // is swallowed on purpose: a missing picture must never cost us the
        // answers themselves.
        try {
          const wpCred = creds?.wordpress;
          const shots = wpCred ? await imageAttachments(outreach, msg.id) : [];
          if (shots[0]) {
            const credit = target.personName + ", " + (target.personRole || target.company);
            const media = await uploadMedia(wpCred, {
              data: shots[0].data,
              contentType: shots[0].contentType,
              filename: shots[0].filename,
              alt: credit,
              caption: credit + ". Picture: supplied",
            });
            if (media?.url) {
              data.headshotUrl = media.url;
              out.notes.push(target.company + ": headshot saved");
            }
          }
        } catch (e) {
          out.notes.push(target.company + ": headshot not saved, " + String(e.message).slice(0, 60));
        }
      } else {
  ...
  const agreed = await db.interviewTarget.findMany({ where: { ...mine, status: "agreed", questionsSentAt: null } });
  ...
  // ---- 2b. the nudge, for anyone sitting on the questions ----
  //
  // JB's timing, 23 Sep 2026: seven days, up from the three to four it ran at
  // until then. Randomised per row off the id so the whole batch does not chase
  // on the same morning, which is the pattern that made the first send read as
  // a mailshot. followUpSentAt guards it to exactly one.
  //
  // People who said yes and then went quiet are chased too, in their own words
  // (buildAgreedChase), not with the silence nudge. They used to be excluded
  // altogether, on the reasoning that the nudge copy would read wrongly to
  // them, and the consequence was that the warmest leads in the whole queue
  // were the only ones nobody ever chased: on 23 Sep 2026 seven were sitting
  // there, two of them three weeks silent after saying yes.
  const waiting = await db.interviewTarget.findMany({
    where: { ...mine, status: "questioned", followUpSentAt: null, questionsSentAt: { not: null } },
  });
  for (const t of waiting) {
    if (out.nudges >= maxSends) break;
    const days = 7 + (hashOf(t.id) % 2); // 7 or 8
    const due = new Date(t.questionsSentAt.getTime() + days * 86400000);
    if (now < due || now < nextSendableTime(due)) continue;
    const body = t.agreedAt
      ? buildAgreedChase({ personName: t.personName, company: t.company, questions: t.questions, titleName, senderName, siteUrl })
      : buildNudge({ personName: t.personName, company: t.company, titleName, senderName, siteUrl });
    try {
      await sendGmail({
        outreach, to: t.email, toName: t.personName,
        subject: stripEmDashes(`Re: ${followUpSubject(t.company)}`),
        text: body, html: htmlise(body),
      });
      await db.interviewTarget.update({ where: { id: t.id }, data: { followUpSentAt: now } });
      out.nudges++;
    } catch (e) {
      await db.interviewTarget.update({ where: { id: t.id }, data: { error: String(e.message).slice(0, 300) } });
    }
  ...
  // ---- 2c. old pre-asks that never got the questions ----
  //
  // The August batch asked permission first and 31 people ignored it. Sending
  // the questions up front is what has worked since, and these rows already
  // carry their seven questions and a checked address, so they get that mail
  // rather than sitting at "asked" forever. Until 23 Sep 2026 nothing in the
  // sweep touched them: 28 rows, one of them a month old.
  const preAsked = await db.interviewTarget.findMany({
    where: { ...mine, status: "asked", questionsSentAt: null, askedAt: { not: null }, email: { not: null } },
  });
  for (const t of preAsked) {
    if (out.questionsSent >= maxSends) break;
    const age = (now - t.askedAt) / 86400000;
    if (age < 7 || !String(t.questions || "").trim()) continue;
    const body = buildFollowUp({
      ...
      siteUrl,
      senderName,
      viaGeneric: isGenericAddress(t.email),
      firstContact: false,
      since: age > 13 ? "a few weeks ago" : "last week",
    });
    try {
      await sendGmail({ outreach, to: t.email, toName: t.personName, subject: followUpSubject(t.company), text: body, html: htmlise(body) });
      await db.interviewTarget.update({
        where: { id: t.id },
        data: { status: "questioned", askBody: body, questionsSentAt: now, error: null },
      });
      out.questionsSent++;
    } catch (e) {
      await db.interviewTarget.update({ where: { id: t.id }, data: { error: String(e.message).slice(0, 300) } });
  ...
  // ---- 3. draft anyone whose answers have arrived ----
  if (draft) {
    const answered = await db.interviewTarget.findMany({ where: { ...mine, status: "answered", articleId: null } });
    for (const t of answered.slice(0, 2)) {
      try {
        const r = await draft(t, { franchise });
        if (r.drafted) {
          out.drafted++;
          out.notes.push(`drafted ${t.company}${r.quotesDropped ? `, ${r.quotesDropped} quote(s) dropped` : ""}`);
        } else {
          out.notes.push(`draft skipped for ${t.company}: ${r.reason}`);
        }
      } catch (e) {
        await db.interviewTarget.update({ where: { id: t.id }, data: { error: String(e.message).slice(0, 300) } });
        out.notes.push(`draft failed for ${t.company}`);
      }
    }
  }

  // ---- 4. the backlink ask, a day after it went live ----
  const live = await db.interviewTarget.findMany({
    where: { ...mine, status: "published", notifiedAt: null, publishedUrl: { not: null } },
  });
  for (const t of live) {
