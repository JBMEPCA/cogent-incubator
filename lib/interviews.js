...
export async function resolveAddress(target, { verify, hunt } = {}) {
  ...
  if (hunt && target.companyDomain) {
    ...
    // A no-reply box is a published address and a dead end. The hunter returned
    // noreply@germinalamenity.com on 2 Sep and nothing upstream would have
    // caught it: "noreply" is not in GENERIC_LOCAL, so it reads as a perfectly
    // ordinary company inbox right up to the point where the mail vanishes and
    // the row sits at "asked" forever looking like silence.
    if (fresh(found?.email) && !NO_REPLY.test(found.email)) {
      return { email: found.email, source: "hunted", verifierBlind: blind };
    }
  }
...
/** Addresses that accept mail and answer nothing. */
export const NO_REPLY = /^(no-?reply|do-?not-?reply|donotreply|bounce|mailer-daemon|postmaster|unsubscribe)@/i;
...
export async function runInterviewSweep(site, { db, creds, anthropic, siteUrl, draft, now = new Date(), maxSends = 5 }) {
  ...
  const waiting = await db.interviewTarget.findMany({
    where: { status: "questioned", followUpSentAt: null, questionsSentAt: { not: null } },
  });
