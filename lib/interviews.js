...
/**
 * The nudge, three to four days after the questions went out.
 *
...
export function buildNudge({ personName, company, titleName, senderName, siteUrl }) {
  ...
  return stripEmDashes(
    [
      `Hi ${first},`,
      "",
      `I sent some questions over a few days ago about featuring ${company} in ${titleName}, so this is just in case it went astray.`,
      "",
...
export async function runInterviewSweep(
  ...
  //
  // JB's timing: three to four days. Randomised per row off the id so the whole
  // batch does not chase on the same morning, which is the pattern that made
  // the first send read as a mailshot. followUpSentAt guards it to exactly one.
  ...
  for (const t of waiting) {
    ...
    const days = 3 + (hashOf(t.id) % 2); // 3 or 4
