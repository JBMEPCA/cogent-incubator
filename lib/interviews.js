...
export async function runInterviewSweep(
  ...
  // agreedAt must be null. Someone who wrote back to say yes is also sitting on
  // the questions, but the nudge is written for silence and would read to them
  // as though we had lost track of our own correspondence.
  const waiting = await db.interviewTarget.findMany({
    where: { ...mine, status: "questioned", followUpSentAt: null, questionsSentAt: { not: null }, agreedAt: null },
  });
  for (const t of waiting) {
    ...
    const body = buildNudge({
      personName: t.personName, company: t.company, titleName, senderName, siteUrl,
    });
    try {
      await sendGmail({
