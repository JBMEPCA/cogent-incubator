...
export async function runInterviewSweep(
  ...
  const waiting = await db.interviewTarget.findMany({
    where: { status: "questioned", followUpSentAt: null, questionsSentAt: { not: null }, agreedAt: null },
  });
  ...
  if (draft) {
    const answered = await db.interviewTarget.findMany({ where: { status: "answered", articleId: null } });
