...
async function fileBrandLinks(debt) {
  ...
  for (const post of debt) {
    for (const brand of post.unlinkedBrands) {
      ...
      const answered = await prisma.seoSuggestion.findFirst({
        where: { title, status: { in: ["pending", "dismissed"] } },
      });
      if (answered) continue;

      await prisma.seoSuggestion.create({
        data: {
          kind: "brand_link",
          wpPostId: post.id,
...
export async function runSeoAudit() {
  if (!isSeoAgentConfigured()) throw new Error("SEO agent not configured");

  const posts = await fetchPosts(AUDIT_POSTS);
  ...
  for (const s of parsed.suggestions || []) {
    ...
    const dupe = await prisma.seoSuggestion.findFirst({
...
export async function applySuggestion(suggestion) {
  ...
  if (suggestion.kind === "title_update" && payload?.newTitle && suggestion.wpPostId) {
    await updatePost(suggestion.wpPostId, { title: payload.newTitle });
    return;
  }
  ...
  if (suggestion.kind === "brand_link" && payload?.anchor && payload?.url && suggestion.wpPostId) {
    const post = await fetchPost(suggestion.wpPostId);
    ...
    await updatePost(suggestion.wpPostId, { content: r.html });
    return;
  }
  ...
  if (
    ...
    await updatePost(suggestion.wpPostId, {
