...
export async function runSeoAudit(site, wp) {
  ...
  const res = await client.messages.create({
    ...
    //
    // The WORK is bounded instead — fewer posts per sweep, less content per
    // post, a hard cap on suggestions and short verbatim finds (see
    // MODEL_POSTS, the content slice below, and AUDIT_SYSTEM). The sweep runs
    // twice a day, so a smaller bite that completes beats a larger one thrown
    // away at the ceiling.
    max_tokens: 20000,
