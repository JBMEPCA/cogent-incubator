...
/**
 * One wave-title's entry, with its colours read from the generated palette.
 *
 * logoWidth is only a fallback: runBriefing measures the real mark with
 * mastheadDisplayWidth() and passes that in. It matters when a title has no
 * mark at all, where leaving it undefined would render width="undefined" and
 * trip the render's own broken-value check.
 */
const waveTitle = (slug, { name, emoji, font, logoWidth, uk }) => {
  const p = paletteFor(slug);
  if (!p) throw new Error(`no palette for ${slug}; run scripts/build-brand-palettes.mjs`);
  return { name, emoji, font: font || p.font, colors: p, logoWidth, uk };
};
...
export const BRIEFING_TITLES = {
  ...
  "golf-resort-magazine": {
    ...
    logoWidth: 210,
    uk: false,
    // What makes a golf reader open the Monday email, per JB on 18 September
    // 2026 after the first proof led with a COO appointment and a pro-shop
    // merchandise guide: "better stories that look sexier". The picker reads
    // this for the featured slot and when choosing among the most read.
    taste:
      "Golf Resort Magazine's Monday email must look aspirational. Favour destination and development stories with a striking photograph: new resorts and courses opening, big renovations and restorations, landmark acquisitions and sales of famous properties, rankings and awards, championship venues, luxury hotel and golf openings. Avoid appointments, software and booking tools, procurement and supplier guides, uniforms, merchandise and back-office operations unless nothing else exists.",
  },
  ...
  // ---- the 18 September 2026 wave ----
  //
  // These five take their colours straight from lib/brand/palettes.js, which is
  // generated from each site's own theme.json and is where newsletter-template.js
  // already reads them from. The five above predate that file and still carry
  // their hex values inline; there is nothing to be gained by copying a third
  // set of literals in behind them.
  //
  // `font` is overridden wherever the site's heading face cannot carry body
  // text. Bebas Neue and Barlow Condensed are condensed all-caps display cuts
  // and Playfair Display is a display serif: in the clients that do load web
  // fonts they turn the email into something you squint at. The masthead image
  // already carries the brand face, so the copy underneath is free to be
  // readable. Nursery and Dental keep theirs, which are text faces already.
  //
  // `uk` follows Site.markets: GB-only titles get the UK wording in the footer
  // basis-for-processing line, multi-market ones the general wording.
  "gym-business-news": waveTitle("gym-business-news", {
    name: "The Monday Warm-Up", emoji: "🏋️", font: "Archivo", logoWidth: 280, uk: false,
  }),
  "nursery-daily": waveTitle("nursery-daily", {
    name: "The Monday Drop-Off", emoji: "🎒", logoWidth: 150, uk: true,
  }),
  "senior-lifestyle-business": waveTitle("senior-lifestyle-business", {
    name: "The Monday Open House", emoji: "🏡", font: "Archivo", logoWidth: 272, uk: false,
  }),
  "dental-business-news": waveTitle("dental-business-news", {
    name: "The Monday Check-Up", emoji: "🦷", logoWidth: 158, uk: true,
  }),
  "smart-farming-news": waveTitle("smart-farming-news", {
    name: "The Monday Market", emoji: "🌾", font: "Archivo", logoWidth: 280, uk: true,
  }),
};
