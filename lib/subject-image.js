...
export const SKIP_HOST = /(^|\.)(google|googleusercontent|facebook|twitter|linkedin|instagram|youtube|youtu|pinterest|doubleclick|gravatar|w3|schema|cloudflare|hubspot|adobe)\.[a-z.]+$/i;
...
export async function fetchText(url, ua) {
...
export function metaImage(html, pageUrl) {
...
export async function fetchImage(url, ua, { minWidth = MIN_WIDTH, minHeight = MIN_HEIGHT } = {}) {
  ...
  return { base64: buf.toString("base64"), buffer: buf, type, filename, size };
}
...
export async function resolveOwnSite(name, ua, log, deadline) {
...
// `subject` may be passed in by a caller that has already classified the story
// (lib/person-image.js does, and stores it on the article), saving the call.
//
// A PERSON is not handled here. This path takes one og:image per page, which on
// an appointment story is the company's banner as often as the person: the
// Toptracer COO story went out on 22 Sep 2026 under Toptracer's site preview.
// People go through lib/person-image.js, which reads every photo on the page
// and never falls back to stock.
export async function subjectImage(site, { title, body, sourceUrl, subject: known = null }) {
  ...
  let subject = known?.kind ? { name: known.name || "", kind: known.kind } : null;
  if (!subject) try {
  ...
  if (subject.kind === "person") {
    log(`${subject.name} is a person - lib/person-image.js handles people`);
    return null;
  }
