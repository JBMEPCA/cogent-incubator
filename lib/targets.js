...
// A page render must not wait on somebody else's API.
//
// Both reads below are cached for 15 minutes, which is worthless on a cache
// MISS: the render simply blocks until Google answers. /api/cron/targets, which
// runs this for five titles, hit its 60 second ceiling on 9 September - about
// twelve seconds a title, which is exactly what JB was seeing when a magazine
// page took fifteen seconds to open.
//
// So the live read gets a deadline and the existing fallback does the rest.
// Every title had an AudienceSnapshot from that same morning carrying real
// visitor and subscriber numbers, so the worst case is a figure a few hours old
// rather than a page nobody wants to open.
//
// The underlying promise is NOT cancelled: it runs on and populates the cache,
// so a slow first visit warms the number for the next one.
function withDeadline(promise, ms, label) {
  return Promise.race([
    promise,
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`${label} did not answer within ${ms}ms; using the last snapshot`)), ms)
    ),
  ]);
}
...
async function collectTargetFacts(ctx, deadlineMs) {
  ...
  if (isGoogleConfigured() && creds.google_analytics?.ga4PropertyId) {
    try {
      visitors = await withDeadline(
        cachedVisitors(creds.google_analytics.ga4PropertyId, sinceIso),
        deadlineMs,
        "GA4"
      );
      visitorsSource = "live";
  ...
  if (creds.mailchimp?.audienceId) {
    try {
      const nl = await withDeadline(
        cachedNewsletterFacts(creds.mailchimp.audienceId),
        deadlineMs,
        "Mailchimp"
      );
 ...
 * anyone is looking.
 */
// deadlineMs is the caller's patience, and it matters more than it looks.
//
// A snapshot is only written when the read came back LIVE, so a page-length
// deadline applied everywhere would quietly stop AudienceSnapshot updating and
// rot the very fallback the pages depend on. The cron has minutes and does the
// live reads; a page has a reader waiting and takes whatever is already cached,
// falling back to the snapshot the cron wrote.
export const TARGETS_PAGE_DEADLINE_MS = 2500;
const CRON_DEADLINE_MS = 25000;

export async function targetBoard(ctx, { record = true, deadlineMs = CRON_DEADLINE_MS } = {}) {
  const facts = await collectTargetFacts(ctx, deadlineMs);
