...
/**
 * Every site that sent us a referral session in the window.
 *
 * Deliberately not cached and not part of fetchAnalytics: this one is read by a
 * sweep that writes what it finds to ReferringDomain, and a cached read would
 * mean a link discovered at 09:05 was recorded as discovered at whatever time
 * the cache last filled.
 *
 * Not a backlink index. It sees a link only once somebody clicks it, so a live
 * link nobody has followed is invisible here — that is the trade for using a
 * credential we already have rather than buying one.
 */
export async function fetchReferrers(ga, days = 28) {
  const propertyId = ga?.ga4PropertyId;
  if (!isGoogleConfigured() || !propertyId) return { available: false, rows: [] };
  ...
  const res = await googlePost(
    ...
    `https://analyticsdata.googleapis.com/v1beta/properties/${String(propertyId).trim()}:runReport`,
    {
      ...
      metrics: [{ name: "sessions" }],
      // Referral only. Without it this returns every organic and email session
      // too, and "google" becomes our biggest backlink.
      dimensionFilter: {
  ...
  // One row per source per landing page; fold to one per source, keeping the
  // page that took the most of its traffic as the clue to which article won it.
  const bySource = new Map();
  for (const row of res.rows || []) {
    const domain = normaliseReferrer(row.dimensionValues[0].value);
    if (!domain) continue;
    const landing = row.dimensionValues[1]?.value || null;
    const sessions = Number(row.metricValues[0].value) || 0;
    const entry = bySource.get(domain) || { domain, sessions: 0, landingPage: landing, topPageSessions: 0 };
    entry.sessions += sessions;
    if (sessions > entry.topPageSessions) {
      entry.topPageSessions = sessions;
      entry.landingPage = landing;
    }
  ...
  return {
    ...
    rows: [...bySource.values()]
      .map(({ topPageSessions, ...r }) => r)
      .sort((a, b) => b.sessions - a.sessions),
  };
}

// Sources that arrive as medium=referral but are not somebody linking to us.
// Every one of these would otherwise be reported as a new backlink the first
// time it appeared, and the report is only worth reading if a new line in it
...
const NOT_A_LINK = new Set([
  ...
  // Search engines. GA4 files a search visit as a referral rather than as
  // organic whenever it cannot classify it — search.google.com is currently
  // Smart SME's ONLY referral row, and it is not a link.
  "search.google.com",
  "google.com",
  "bing.com",
  "duckduckgo.com",
  ...
  // Redirectors on our own distribution. A click on a post we published is not
  // a link somebody else chose to give us.
  "t.co",
  "lnkd.in",
  "l.facebook.com",
  "lm.facebook.com",
  ...
  // Checkout, auth and infrastructure hops, which send the reader back to the
  // page they came from.
  "stripe.com",
  "checkout.stripe.com",
  "paypal.com",
...
/**
 * GA4's sessionSource down to a bare host, or null if it is not one.
 *
 * The dimension is not a hostname field: it carries whatever utm_source a
 * campaign was tagged with, so "newsletter-aug" and "linkedin_bio" arrive here
 * alongside real domains. Anything without a dot is a tag, not a site.
 */
function normaliseReferrer(raw) {
  const value = String(raw || "").trim().toLowerCase();
  if (!value || !value.includes(".")) return null;
  let host = value;
  try {
    if (value.includes("/")) host = new URL(value.startsWith("http") ? value : `https://${value}`).hostname;
  } catch {
    return null;
  }
  host = host.replace(/^www\./, "").replace(/\.$/, "");
  if (!/^[a-z0-9.-]+\.[a-z]{2,}$/.test(host)) return null;
  return NOT_A_LINK.has(host) ? null : host;
}
