...
{"verdict": "yes"|"no", "score": <0-100 relevance>, "reason": "<one sentence>", "alt": "<SEO alt text under 120 chars describing what is actually visible, including the keyphrase if it fits naturally>"}`,
  ...
  return {
    // The verdict decides, not the score. A yes with a 55 on it used to be a
    // rejection nobody could see: the reason field said the picture was fine,
    // the article stayed bare, and the desk went round again. The score is a
    // confidence note for the log, and the four hard refusals are expressed as
    // "no" - so a yes is a yes.
    ok: verdict === "yes",
    score,
 ...
 */
// effort: optional output_config effort. The press desk passes "medium": its
// quotes and figures are already proven by code before the gate sees the piece.
export async function reviewArticle({ site, title, body, type, keyphrase, metaDesc, brief, effort }) {
  ...
  // The headline length rule, enforced rather than requested. It has sat in
  // every title's editorial standard since 11 September 2026, and the prompt
  // alone got 11 of 12 new drafts inside it. The twelfth, and every draft
  // already banked before the rule, went out at 78 to 96 characters before the
  // colon, where Google cuts it off. The repair pass may rewrite the headline,
  // so this costs one Sonnet call, never a kill.
  const lead = String(title || "").split(":")[0].trim();
  if (lead.length > 65)
    issues.push(
      `Headline runs ${lead.length} characters before any colon (ceiling 65, aim 60): Google truncates it. Put who, what and the number first and cut the rest`
    );
