...
${fromSource ? "" : `This is a STOCK or PRESS-LIBRARY photograph. It will almost never show the specific company, club, product, site or person the article names, and it is not meant to. A trade title illustrates a story about a named operator with a representative photograph of the activity: that is correct practice, not a fault. Judge the picture against the article's SUBJECT MATTER, never against its ability to prove which organisation it depicts.`}
...
REJECT (verdict "no") if ANY of these are true:${fromSource ? "" : "
...
- THE IMAGE CARRIES A WATERMARK. This one is absolute and has no exceptions. A stock library's mark, a tiled or diagonal repeat of an agency name, "Shutterstock", "Getty", "Alamy", "iStock", "Dreamstime", a photographer's signature stamped across the frame, a sample or preview overlay, a semi-transparent wordmark anywhere on the picture: reject it, however good the photograph is and however well it fits. This is JB's single standing rule on imagery, given on 30 August 2026 alongside an explicit instruction to disregard every other rights consideration, so it is the one thing this gate must never wave through.
- The image contains text or captions that would confuse or mislead a reader. A company's own logo on its own product is NOT this, and never a reason to reject.
...
A PRODUCT OR SERVICE LAUNCH is the same problem wearing a different hat. A stock library does not stock the launch: it has no photograph of a named shear, a named hair-replacement system or a named email tool. The correct header is the trade at work - the chair, the counter, the workshop, the desk - and "wrong product category", "does not demonstrate the product" and "shows a different kind of tool" are NOT grounds for rejection. On 28 August 2026 this gate refused a barber using clippers, for a barbering product launch, on exactly that reasoning, and the article did not publish.
