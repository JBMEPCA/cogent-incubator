...
- Each line its own paragraph. Then two or three short lines saying what the story is and why it matters to the reader. Use ONLY facts in the headline and standfirst given. Never add a number, name or claim that is not there.
...
Reply with ONLY JSON, no code fence: {"id": <numeric id>, "caption": "..."}`,
    ...
    try {
      ...
      caption = noEmDash(String(j.caption || ""))
        .split(/\n+/)
        .map((line) => line.trim())
        .filter(Boolean)
        .join("\n⠀\n");
