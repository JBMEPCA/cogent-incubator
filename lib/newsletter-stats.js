 ...
 */
export async function issueHistory(limit = 24) {
  const data = await mc(`/reports?count=200&type=regular&sort_field=send_time&sort_dir=DESC`);
  return (data.reports ?? [])
    .filter((r) => r.list_id === AUDIENCE_ID && r.emails_sent > 0)
    .slice(0, limit)
    .map((r) => {
...
export async function growthHistory(months = 12) {
  const d = await mc(`/lists/${AUDIENCE_ID}/growth-history?count=${months}&sort_field=month&sort_dir=DESC`);
  return (d.history ?? [])
    .map((h) => ({
      month: h.month,
...
export async function newsletterReport(siteId) {
  const db = forSite(siteId);
  const [list, issues] = await Promise.all([
    mc(`/lists/${AUDIENCE_ID}?fields=name,stats.member_count,stats.unsubscribe_count,stats.cleaned_count`).catch(() => null),
    issueHistory().catch(() => []),
  ]);
