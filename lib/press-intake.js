...
import { fromLine, PUSH_MARK } from "./press-desk";
...
// Every state label except the one being applied.
//
// The desk only ever ADDED a label, which was right while a release passed
// through once and stopped. A pushed release passes through twice, so without
// this a piece that was Skipped and is now Published wears both, and the
// Gmail folder — the thing a human actually reads — stops meaning anything.
const otherStateLabels = (labels, keep) =>
  STATE_LABELS.filter((n) => n !== keep && n !== "Press/Replied")
    .map((n) => labels.get(n))
    .filter(Boolean);
 ...
 * the client's site, not the agency's: a PR firm's coverage page lists everyone
 * they have ever placed and is worth close to nothing next to a referring
 * domain in the trade we publish in.
 *
 * The website is the reliable test and is tried first. When the sorter did not
 * find one, which is common, the company NAME is compared against the sending
 * domain instead: Michaella at ajc93.com writing about Enki Towels shares no
 * word with it, so she is an agency, and that release went out in September
 * addressed as though she were the brand because the website was blank and the
 * test gave up there.
 *
 * Unknown still counts as "not an agency". Asking a brand to pass a request to
 * their own client reads as a mistake, and that is the worse way to be wrong.
 */
function looksLikeAgency(to, sorted) {
  const from = hostOf(to?.email);
  if (!from) return false;
  const brand = hostOf(sorted?.companyWebsite);
  if (brand) return from !== brand && !from.endsWith(`.${brand}`) && !brand.endsWith(`.${from}`);

  // No website on file. Compare the sending domain's own label against the
  // words of the company name, both stripped to letters and digits, so
  // "Bright Horizons" still matches brighthorizons.com and a shortened or
  // hyphenated domain still matches the word it was shortened from.
  const label = from.split(".")[0].replace(/[^a-z0-9]/g, "");
  const name = String(sorted?.company || "").toLowerCase();
  if (!label || !name) return false;
  const parts = name.replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter((w) => w.length > 2);
  if (!parts.length) return false;
  const joined = parts.join("");
  return !(label.includes(joined) || joined.includes(label) || parts.some((w) => label.includes(w)));
}
...
export function replyLines({ site, to, url, sorted }) {
  ...
  return [
    ...
    "",
    agency
      ? `If ${company} keep a news or press page, a link to that article is all we are after.`
...
/**
 * One release, from email to published post.
 *
 * `claimed` and `override` are the human push — pushRelease below, and the
 * Press Releases page it serves. A pushed release skips the SELECTION
 * judgements (relevance, news value, whether another title already has it)
 * because a person has just overruled them. It does NOT skip the quality gate:
 * what JB is overriding is our taste, not the check that every quote is the
 * sender's own words and every figure is in the release.
 */
async function processRelease({ site, db, wp, token, labels, press, msg, client, authorId, claimed = null, override = false, force = false }) {
  const L = (name) => labels.get(name);
  const link = `gmail:${msg.id}`;
  const title = cleanSubject(msg.subject) || "(untitled release)";
  // Who wrote to us, stamped onto the row so the Press Releases page can name
  // them without going back to Gmail for every line it draws. The desk has
  // always known this and never kept it.
  const sender = replyAddress(msg, null);
  const stamp = (text) => [fromLine(sender), text].filter(Boolean).join("\n").slice(0, 20000);
  ...
  let item = claimed;
  if (item) {
    // A push re-runs a row that already exists. Back to "new" for the duration,
    // so a tick landing mid-push sees work in flight rather than a free release.
    await db.feedItem.update({ where: { id: item.id }, data: { status: "new", summary: stamp(msg.text) } });
  } else {
    try {
      item = await db.feedItem.create({
        data: { brandId: brand0.id, title, link, summary: stamp(msg.text), publishedAt: msg.date, status: "new" },
      });
    } catch {
  ...
  const skip = async (reason) => {
    await db.feedItem.update({ where: { id: item.id }, data: { status: "dismissed", summary: `[skipped: ${reason}]\n${stamp(msg.text.slice(0, 19000))}` } });
    await setLabels(token, msg.id, [L("Press/Skipped")], otherStateLabels(labels, "Press/Skipped"));
  ...
  if (msg.pdfs.length) {
    ...
    await db.feedItem.update({ where: { id: item.id }, data: { summary: stamp(msg.text) } });
  }
  ...
  const dup = override
    ? null
    : await fleetRead().feedItem.findFirst({
        where: {
          id: { not: item.id },
  ...
  // Still sorted on a push: the company, the category, the embargo and the
  // reply address all come out of here, and it costs a fifth of a penny. Only
  // the thresholds below are skipped.
  ...
  //
  // A push does not change this. `force` does, and only a person can set it:
  // the Press Releases page refuses the first click on an unreadable embargo,
  // says so, and offers a second button. Breaking an embargo gets a title
  // dropped from the list for good; missing one by an hour costs nothing.
  let goLive = null;
  if (sorted.embargo?.present) {
    ...
    if (!sorted.embargo.clear || Number.isNaN(t.getTime())) {
      if (!force) {
        ...
        await setLabels(token, msg.id, [L("Press/Needs review")], otherStateLabels(labels, "Press/Needs review"));
        return { held: "embargo time could not be read with certainty", embargo: true, subject: title.slice(0, 80) };
      }
    } else if (t > new Date(Date.now() + 5 * 60000)) goLive = t;
  }
  ...
  // A push re-uses the article the desk already started on this release rather
  // than making a second one. Held releases already have a draft, a brief and a
  // QA report attached to a row; leaving that behind would litter the title's
  // article list with an unreachable idea for every push.
  const started = claimed
    ? await db.article.findFirst({ where: { sourceItemId: item.id, status: "idea" }, orderBy: { createdAt: "desc" } })
    : null;
  const articleData = {
    data: {
      title,
...
${msg.text.slice(0, 12000)}`
  ...
  const article = started
    ? await db.article.update({ where: { id: started.id }, data: { ...articleData.data, qaPassed: false, qaReport: null } })
  ...
  if (goLive) {
    await setLabels(token, msg.id, [L("Press/Scheduled")], otherStateLabels(labels, "Press/Scheduled"));
    return { scheduled: post.link, goLive: goLive.toISOString(), photo, subject: title.slice(0, 80), article: a.id };
  }
  await setLabels(token, msg.id, [L("Press/Published")], otherStateLabels(labels, "Press/Published"));
...
// A push in flight. The Press Releases page finds these by the feed item id and
// draws the row as working rather than as a button to press again.
//
// This exists because the first version of the push answered the browser only
// when the whole job was finished, a minute or more later, and that answer
// never arrived: both of JB's first two pushes ran to completion — one
// published, one correctly held again — while the page sat on "Writing it
// up..." for ever. Whatever drops a request held open that long behind the auth
// proxy, waiting on one was the wrong shape. So the work is now started, the
// browser is answered immediately, and the outcome is read back out of the
// database. The open AgentRun IS the progress, and lib/agents/runtime.js
// reapStaleRuns already closes one whose function was killed. PUSH_MARK itself
// lives in press-desk.js, which this module already imports from.
/**
 * Check a release can be pushed, and claim it. Fast: no model calls, no Gmail.
 *
 * Everything that can sensibly be refused is refused HERE, while there is still
 * a person watching, so the reasons a push cannot happen reach them as an
 * answer rather than as a row that silently never changes.
 */
export async function beginPush({ feedItemId } = {}) {
  // findFirst, not findUnique: fleetRead() allows only filter-shaped reads on a
  // tenanted table, because a unique lookup cannot have a siteId folded into
  // its `where` and so cannot be proved to belong to one title. A cuid is
  // unique anyway, so the filter form is the same lookup. The site this row
  // turns out to belong to is what every write below is then scoped to.
  const item = await fleetRead().feedItem.findFirst({
    where: { id: feedItemId },
    include: { site: { select: { slug: true } }, articles: { select: { id: true, status: true, wpPostId: true } } },
  });
  if (!item) return { error: "That release is no longer in the database." };
  if (!item.link.startsWith("gmail:")) return { error: "That row did not come from a press@ mailbox." };
  if (item.link.startsWith("gmail:dry-")) return { error: "That row is a dry-run leftover, not a real email." };
  if (item.articles.some((a) => a.status === "published")) return { error: "This release is already published." };

  const { getSiteContext } = await import("./site");
  const ctx = await getSiteContext(item.site.slug);
  if (!ctx) return { error: "That title is no longer set up." };
  const { site, db, creds } = ctx;
  if (!isWordPressConfigured(creds.wordpress)) return { error: `${site.name} has no WordPress credentials.` };
  if (!creds.outreach?.fromEmail) return { error: `${site.name} has no mailbox configured.` };

  // Two clicks on one release would pay twice to write it and could publish it
  // twice. An open run is the lock.
  const running = await db.agentRun.findFirst({
    where: { trigger: "press-push", endedAt: null, summary: { startsWith: `${PUSH_MARK}${item.id}` }, startedAt: { gte: new Date(Date.now() - 10 * 60000) } },
  });
  if (running) return { error: "This one is already being written. Give it a minute." };

  await ensureAgents(site.id);
  const run = await db.agentRun.create({
    data: { agentKey: "editor", trigger: "press-push", summary: `${PUSH_MARK}${item.id} ${item.title}`.slice(0, 200), ok: false, endedAt: null },
  });
  return { runId: run.id, feedItemId: item.id, slug: item.site.slug, title: item.title };
}

/**
 * Put a release through the desk again because a person said so.
 *
 * The desk turns down roughly half of what arrives, and that judgement has
 * never been visible to anyone. As the titles go majority PR-fed, a wrong no is
 * a story we simply didn't run, and nobody would ever know. So the Press
 * Releases page lists every decision and this is the way back: it re-reads the
 * original email from Gmail, rewrites it, and publishes.
 *
 * What a push overrides: relevance, news value, "not a release", and the
 * duplicate checks. What it does not: the quote and figure checks, and the QA
 * gate. A piece that still fails those comes back held, with the reason, which
 * is the honest answer — the objection was never about whether we wanted it.
 *
 * `force` additionally publishes a release whose embargo could not be read. It
 * is a second, separate click for exactly that case.
 *
 * Runs AFTER the browser has been answered (see beginPush). Nothing here may
 * assume anyone is still listening: every outcome is written to the row.
 */
export async function runPush({ runId, feedItemId, slug }, force = false) {
  DRY = false;
  const { getSiteContext } = await import("./site");
  const { titleAddresses } = await import("./inbox-labels");
  ...
  let error = null;
  try {
    // replyAddress refuses to thank one of our own addresses, and the set that
    // tells it so is filled in by the tick. A push has no tick.
    ...
    const token = await getGoogleAccessToken(SCOPES, mailbox);
    const labels = await labelIds(token);
    const msgId = item.link.slice("gmail:".length);
    const msg = readMessage(await gmail(token, `/messages/${msgId}?format=full`));
    const authorId = await authorForSite(wp, site);

    try {
      result = await runMetered(meter, () =>
        processRelease({ site, db, wp, token, labels, press, msg, client: new Anthropic(), authorId, claimed: item, override: true, force })
      );
    } catch (e) {
      error = e.message?.slice(0, 500) || String(e);
      result = { error };
      await setLabels(token, msgId, [labels.get("Press/Needs review")], otherStateLabels(labels, "Press/Needs review")).catch(() => {});
    }
  } catch (e) {
    // Gmail, credentials, the mailbox: anything before the release itself.
    error = e.message?.slice(0, 500) || String(e);
    result = { error };
  }

  // Close the run that beginPush opened. Its own trigger, so a pushed piece is
  // still counted against the title's spend and cost per article but can be
  // told apart from the desk's own work.
  await db.agentRun.update({
    where: { id: runId },
    data: {
      summary: `Press desk (pushed by hand): ${result.published ? "published" : result.scheduled ? "scheduled" : result.held ? "held" : result.skipped ? "skipped" : "failed"}: ${String(result.subject || item?.title || "").slice(0, 120)}${result.held ? ` (${result.held})` : ""}${result.photo ? ` [photo: ${result.photo}]` : ""}`,
      ok: !error,
      error,
      model: meter.model,
      inputTokens: meter.input,
      outputTokens: meter.output,
      costUsd: meter.cost,
      articleId: result.article || null,
      endedAt: new Date(),
    },
  });
  if (result.article) await db.article.update({ where: { id: result.article }, data: { costUsd: meter.cost } });
  return { ...result, costUsd: Number(meter.cost.toFixed(4)) };
}
