 ...
 * of earning it.
 */
export function replyLines({ site, to, url, sorted, title }) {
  ...
  const anchor = String(title || site.name).replace(/<[^>]+>/g, "").trim() || site.name;
  ...
  return [
    ...
    "",
    agency
      ? `If you send ${company} a coverage round-up, this is the URL to include, and a link from their own site is worth far more to us than one from anywhere else. Here is a line either of you can paste into a news or press page:`
      : "If you keep a news, press or coverage page, please add the article to it. Here is a line you can paste straight in:",
    "",
    `<a href="${url}">${anchor}</a>, ${site.name}`,
    "",
    `A link from ${agency ? "their site" : "your site"} is what pays for the next piece of coverage, for ${company} and for everyone else who sends us something. It takes a minute and it makes a real difference to a title this young. A mention on LinkedIn is welcome too, though the link on the site is the part that lasts.`,
    "",
...
function replyMime({ site, press, msg, to, url, sorted, title }) {
  const lines = replyLines({ site, to, url, sorted, title });
  const text = lines.join("\n");
  const html = lines
    .map((l) => {
      if (l === url) return `<a href="${url}">${url}</a>`;
      // The paste-ready anchor is shown as markup rather than rendered, so they
      // can copy the tag itself instead of a live link whose href they cannot see.
      if (l.startsWith("<a href=")) return `<code>${l.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</code>`;
      return l;
    })
    .join("<br>\n");
...
async function sendReply(token, { site, press, msg, to, url, sorted, title, mode }) {
  const raw = Buffer.from(replyMime({ site, press, msg, to, url, sorted, title })).toString("base64url");
...
${msg.text.slice(0, 12000)}`
  ...
  const replied = await replyOnce({ site, token, labels, press, msg, sorted, url: post.link, title: stripEmDashes(a.title) });
  return { published: post.link, replied, photo, subject: title.slice(0, 80), article: a.id };
}

async function replyOnce({ site, token, labels, press, msg, sorted, url, title }) {
  const mode = await linkAskMode();
  if (mode === "off") return false;
  const to = replyAddress(msg, sorted);
  if (!to) return false;
  try {
    const how = await sendReply(token, { site, press, msg, to, url, sorted, title, mode });
...
async function sweepScheduled({ site, db, wp, token, labels, press }) {
  ...
  for (const { id } of list.messages || []) {
    ...
    const replied = await replyOnce({ site, token, labels, press, msg, sorted: { company: item.title }, url: post.link, title: post.title?.rendered || item.title });
