...
import { fromLine } from "./press-desk";
 ...
 */
export async function pushRelease({ feedItemId, force = false } = {}) {
  DRY = false;
  ...
  const wp = creds.wordpress;
  if (!isWordPressConfigured(wp)) return { error: `${site.name} has no WordPress credentials.` };
  ...
  let msg;
  try {
    msg = readMessage(await gmail(token, `/messages/${msgId}?format=full`));
  } catch (e) {
    // Deleted from Gmail, or a mailbox we can no longer read. The row stays.
    return { error: `The original email could not be read: ${e.message?.slice(0, 160)}` };
  }
  ...
  // Its own trigger, so a pushed piece is still counted against the title's
  // spend and per-article cost but can be told apart from the desk's own work.
  await db.agentRun.create({
    data: {
      agentKey: "editor",
      trigger: "press-push",
